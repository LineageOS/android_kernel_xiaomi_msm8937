...
int legacy_msm_jpeg_get_clock_index(struct msm_jpeg_device *pgmn_dev,
	const char *clk_name)
{
	uint32_t i = 0;
...
int legacy_msm_jpeg_platform_set_clk_rate(struct msm_jpeg_device *pgmn_dev,
		long clk_rate)
{
	int rc = 0;
	...
	msm_jpeg_idx = legacy_msm_jpeg_get_clock_index(pgmn_dev,
		"core_clk");
	if (msm_jpeg_idx < 0)  {
	...
	/* set the rate */
	legacy_msm_camera_clk_set_rate(&pgmn_dev->pdev->dev,
		pgmn_dev->jpeg_clk[msm_jpeg_idx], clk_rate);

	return rc;
}

void legacy_msm_jpeg_platform_p2v(int iommu_hdl, int fd)
{
	legacy_cam_smmu_put_phy_addr(iommu_hdl, fd);
	return;
}

uint32_t legacy_msm_jpeg_platform_v2p(struct msm_jpeg_device *pgmn_dev, int fd,
	uint32_t len, int iommu_hdl)
{
	...
	rc = legacy_cam_smmu_get_phy_addr(pgmn_dev->iommu_hdl, fd, CAM_SMMU_MAP_RW,
			&paddr, &size);
	...
	return paddr;
err_size:
	legacy_cam_smmu_put_phy_addr(pgmn_dev->iommu_hdl, fd);
err_get_phy:
	return 0;
}
	 ...
	 void *jpeg_vbif_base)
{
	...
	if (pgmn_dev->hw_version != JPEG_8974_V1)
		legacy_msm_camera_io_w(0x00000003,
				jpeg_vbif_base + JPEG_VBIF_ROUND_ROBIN_QOS_ARB);
	else
		legacy_msm_camera_io_w(0x00000001,
				jpeg_vbif_base + JPEG_VBIF_ROUND_ROBIN_QOS_ARB);

	legacy_msm_camera_io_w(0x22222222,
	...
	if (dt_count != 0) {
		...
		for (i = 0; i < dt_count; i = i + 2) {
			JPEG_DBG("%s:%d] %pK %08x\n",
					...
					dt_reg_settings[i + 1]);
			legacy_msm_camera_io_w(dt_reg_settings[i + 1],
					base + dt_reg_settings[i]);
		}
		kfree(dt_reg_settings);
	}
...
{
	int rc;
	rc = legacy_cam_smmu_ops(pgmn_dev->iommu_hdl, CAM_SMMU_ATTACH);
	if (rc < 0) {
...
static int msm_jpeg_detach_iommu(struct msm_jpeg_device *pgmn_dev)
{
	JPEG_DBG("%s:%d] handle %d detach\n",
			__func__, __LINE__, pgmn_dev->iommu_hdl);
	legacy_cam_smmu_ops(pgmn_dev->iommu_hdl, CAM_SMMU_DETACH);
	return 0;
}


int legacy_msm_jpeg_platform_init(irqreturn_t (*handler)(int, void *),
	void *context)
{
	int rc = -1;
	...
	rc = legacy_msm_camera_regulator_enable(pgmn_dev->jpeg_vdd,
	...
	/* enable all clocks */
	rc = legacy_msm_camera_clk_enable(&pgmn_dev->pdev->dev,
	...
	rc = legacy_msm_camera_register_irq(pgmn_dev->pdev,
	...
	pgmn_dev->hw_version = legacy_msm_camera_io_r(pgmn_dev->base +
		JPEG_HW_VERSION);
...
err_fail_iommu:
	legacy_msm_camera_clk_enable(&pdev->dev, pgmn_dev->jpeg_clk_info,
		pgmn_dev->jpeg_clk, pgmn_dev->num_clk, false);
err_clk_enable:
	legacy_msm_camera_regulator_enable(pgmn_dev->jpeg_vdd,
		pgmn_dev->num_reg, false);
err_reg_enable:
	return rc;
}

int legacy_msm_jpeg_platform_setup(struct msm_jpeg_device *pgmn_dev)
{
	int rc = -1;
	...
	jpeg_base = legacy_msm_camera_get_reg_base(pdev, "jpeg_hw", true);
	if (!jpeg_base) {
	...
	vbif_base = legacy_msm_camera_get_reg_base(pdev, "jpeg_vbif", false);
	if (!vbif_base) {
	...
	jpeg_irq_res = legacy_msm_camera_get_irq(pdev, "jpeg");
	if (!jpeg_irq_res) {
	...
	rc = legacy_msm_camera_get_clk_info(pdev, &pgmn_dev->jpeg_clk_info,
	...
	rc = legacy_msm_camera_get_regulator_info(pdev, &pgmn_dev->jpeg_vdd,
		&pgmn_dev->num_reg);
	if (rc < 0) {
	...
	rc = legacy_msm_camera_register_bus_client(pgmn_dev->pdev,
			pgmn_dev->bus_client);
	if (rc < 0) {
	...
	pgmn_dev->res_size = legacy_msm_camera_get_res_size(pdev, "jpeg_hw");
...
err_res_size:
	legacy_msm_camera_unregister_bus_client(pgmn_dev->bus_client);
err_reg_bus:
	legacy_msm_camera_put_regulators(pdev, &pgmn_dev->jpeg_vdd,
		pgmn_dev->num_reg);
err_jpeg_get_reg:
	legacy_msm_camera_put_clk_info(pdev, &pgmn_dev->jpeg_clk_info,
		&pgmn_dev->jpeg_clk, pgmn_dev->num_clk);
err_jpeg_clk:
err_jpeg_irq_res:
	legacy_msm_camera_put_reg_base(pdev, vbif_base, "jpeg_vbif", false);
err_vbif_base:
	legacy_msm_camera_put_reg_base(pdev, jpeg_base, "jpeg_hw", true);
out:
	return rc;
}

void legacy_msm_jpeg_platform_cleanup(struct msm_jpeg_device *pgmn_dev)
{
	/* unregister the bus client */
	legacy_msm_camera_unregister_bus_client(pgmn_dev->bus_client);
	/* release the regulators */
	legacy_msm_camera_put_regulators(pgmn_dev->pdev, &pgmn_dev->jpeg_vdd,
		pgmn_dev->num_reg);
	/* release all the clocks */
	legacy_msm_camera_put_clk_info(pgmn_dev->pdev, &pgmn_dev->jpeg_clk_info,
		&pgmn_dev->jpeg_clk, pgmn_dev->num_clk);
	/* release the jpeg device memory */
	legacy_msm_camera_put_reg_base(pgmn_dev->pdev, pgmn_dev->vbif_base,
		"jpeg_vbif", false);
	/* release the jpeg vbif device memory */
	legacy_msm_camera_put_reg_base(pgmn_dev->pdev, pgmn_dev->base,
		"jpeg_hw", true);
}

int legacy_msm_jpeg_platform_release(void *context)
{
	int result = 0;
	...
	/* release the irq */
	legacy_msm_camera_unregister_irq(pgmn_dev->pdev,
	...
	legacy_msm_camera_clk_enable(&pgmn_dev->pdev->dev, pgmn_dev->jpeg_clk_info,
		pgmn_dev->jpeg_clk, pgmn_dev->num_clk, false);
...
/*
 * legacy_msm_jpeg_platform_set_dt_config() - set jpeg device tree configuration.
 * @pgmn_dev: Pointer to jpeg device.
 *
 ...
 * Return: 0 on success and negative error on failure.
 */
int legacy_msm_jpeg_platform_set_dt_config(struct msm_jpeg_device *pgmn_dev)
{
	int rc = 0;
