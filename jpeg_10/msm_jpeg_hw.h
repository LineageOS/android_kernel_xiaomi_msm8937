...
void *legacy_m_msm_jpeg_hw_pingpong_active_buffer(struct msm_jpeg_hw_pingpong
	*pingpong_hw);

void legacy_m_msm_jpeg_hw_irq_clear(uint32_t, uint32_t, void *);
void legacy_m_msm_jpegdma_hw_irq_clear(uint32_t, uint32_t, void *);
int legacy_m_msm_jpeg_hw_irq_get_status(void *);
int legacy_m_msm_jpegdma_hw_irq_get_status(void *);
long legacy_m_msm_jpeg_hw_encode_output_size(void *);
...
void legacy_m_msm_jpeg_hw_fe_mmu_prefetch(struct msm_jpeg_hw_buf *buf, void *,
	uint8_t decode_flag);
void legacy_m_msm_jpeg_hw_we_mmu_prefetch(struct msm_jpeg_hw_buf *buf, void *,
	uint8_t decode_flag);
void legacy_m_msm_jpegdma_hw_fe_mmu_prefetch(struct msm_jpeg_hw_buf *buf, void *);
void legacy_m_msm_jpegdma_hw_we_mmu_prefetch(struct msm_jpeg_hw_buf *buf, void *);

void legacy_m_msm_jpeg_hw_fe_start(void *);
void msm_jpeg_hw_clk_cfg(void);

void legacy_m_msm_jpeg_hw_reset(void *base, int size);
void msm_jpeg_hw_irq_cfg(void);

uint32_t legacy_m_msm_jpeg_hw_read(struct msm_jpeg_hw_cmd *, void *);
void legacy_m_msm_jpeg_hw_write(struct msm_jpeg_hw_cmd *, void *);
int legacy_m_msm_jpeg_hw_wait(struct msm_jpeg_hw_cmd *, int, void *);
void legacy_m_msm_jpeg_hw_delay(struct msm_jpeg_hw_cmd *, int);
int legacy_m_msm_jpeg_hw_exec_cmds(struct msm_jpeg_hw_cmd *, uint32_t ,
	uint32_t , void *);
