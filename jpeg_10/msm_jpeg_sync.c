...
inline void legacy_msm_jpeg_q_init(char const *name, struct msm_jpeg_q *q_p)
{
...
inline void *legacy_msm_jpeg_q_out(struct msm_jpeg_q *q_p)
{
	unsigned long flags;
...
inline int legacy_msm_jpeg_q_in(struct msm_jpeg_q *q_p, void *data)
{
	unsigned long flags;
...
inline int legacy_msm_jpeg_q_in_buf(struct msm_jpeg_q *q_p,
	struct msm_jpeg_core_buf *buf)
{
	...
	legacy_msm_jpeg_q_in(q_p, buf_p);
	return 0;
}

inline int legacy_msm_jpeg_q_wait(struct msm_jpeg_q *q_p)
{
...
inline int legacy_msm_jpeg_q_wakeup(struct msm_jpeg_q *q_p)
{
	JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
	wake_up(&q_p->wait);
	return 0;
...
inline int legacy_msm_jpeg_q_unblock(struct msm_jpeg_q *q_p)
{
	JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
	q_p->unblck = 1;
	wake_up(&q_p->wait);
...
inline void legacy_msm_jpeg_outbuf_q_cleanup(struct msm_jpeg_device *pgmn_dev,
	struct msm_jpeg_q *q_p)
{
	struct msm_jpeg_core_buf *buf_p;
	JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
	do {
		buf_p = legacy_msm_jpeg_q_out(q_p);
		if (buf_p) {
			legacy_msm_jpeg_platform_p2v(pgmn_dev->iommu_hdl,
					buf_p->ion_fd);
			JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
			kfree(buf_p);
		}
...
inline void legacy_msm_jpeg_q_cleanup(struct msm_jpeg_q *q_p)
{
	void *data;
	JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
	do {
		data = legacy_msm_jpeg_q_out(q_p);
		if (data) {
			JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
			kfree(data);
		}
...
int legacy_msm_jpeg_framedone_irq(struct msm_jpeg_device *pgmn_dev,
	struct msm_jpeg_core_buf *buf_in)
{
	...
	if (buf_in) {
		...
		rc = legacy_msm_jpeg_q_in_buf(&pgmn_dev->evt_q, buf_in);
	} else {
	...
	if (buf_in)
		rc = legacy_msm_jpeg_q_wakeup(&pgmn_dev->evt_q);

	return rc;
}

int legacy_msm_jpeg_evt_get(struct msm_jpeg_device *pgmn_dev,
	void __user *to)
{
	struct msm_jpeg_core_buf *buf_p;
	...
	legacy_msm_jpeg_q_wait(&pgmn_dev->evt_q);
	buf_p = legacy_msm_jpeg_q_out(&pgmn_dev->evt_q);
	...
	if (ctrl_cmd.type == MSM_JPEG_EVT_SESSION_DONE) {
		...
		legacy_msm_camera_update_bus_vector(pgmn_dev->bus_client, 0);
...
int legacy_msm_jpeg_evt_get_unblock(struct msm_jpeg_device *pgmn_dev)
{
	JPEG_DBG("%s:%d] Enter\n", __func__, __LINE__);
	legacy_msm_jpeg_q_unblock(&pgmn_dev->evt_q);
	return 0;
}

void legacy_msm_jpeg_reset_ack_irq(struct msm_jpeg_device *pgmn_dev)
{
	JPEG_DBG("%s:%d]\n", __func__, __LINE__);
}

void legacy_msm_jpeg_err_irq(struct msm_jpeg_device *pgmn_dev,
	int event)
{
	int rc = 0;
	struct msm_jpeg_core_buf buf;
	...
	rc = legacy_msm_jpeg_q_in_buf(&pgmn_dev->evt_q, &buf);
	if (!rc)
		rc = legacy_msm_jpeg_q_wakeup(&pgmn_dev->evt_q);
...
int legacy_msm_jpeg_we_pingpong_irq(struct msm_jpeg_device *pgmn_dev,
	struct msm_jpeg_core_buf *buf_in)
{
	int rc = 0;
	...
	if (buf_in) {
		JPEG_DBG("%s:%d] 0x%08x %d\n", __func__, __LINE__,
			(int) buf_in->y_buffer_addr, buf_in->y_len);
		rc = legacy_msm_jpeg_q_in_buf(&pgmn_dev->output_rtn_q, buf_in);
	} else {
	...
	buf_out = legacy_msm_jpeg_q_out(&pgmn_dev->output_buf_q);

	if (buf_out) {
		JPEG_DBG("%s:%d] 0x%08x %d\n", __func__, __LINE__,
			(int) buf_out->y_buffer_addr, buf_out->y_len);
		rc = legacy_msm_jpeg_core_we_buf_update(pgmn_dev, buf_out);
		kfree(buf_out);
	} else {
		legacy_msm_jpeg_core_we_buf_reset(pgmn_dev, buf_in);
		JPEG_DBG("%s:%d] no output buffer\n", __func__, __LINE__);
		rc = -2;
	}

	if (buf_in)
		rc = legacy_msm_jpeg_q_wakeup(&pgmn_dev->output_rtn_q);

	return rc;
}

int legacy_msm_jpeg_output_get(struct msm_jpeg_device *pgmn_dev, void __user *to)
{
	struct msm_jpeg_core_buf *buf_p;
	...
	legacy_msm_jpeg_q_wait(&pgmn_dev->output_rtn_q);
	buf_p = legacy_msm_jpeg_q_out(&pgmn_dev->output_rtn_q);
	...
	buf_cmd = buf_p->vbuf;
	legacy_msm_jpeg_platform_p2v(pgmn_dev->iommu_hdl, buf_p->ion_fd);
	kfree(buf_p);
...
int legacy_msm_jpeg_output_get_unblock(struct msm_jpeg_device *pgmn_dev)
{
	JPEG_DBG("%s:%d] Enter\n", __func__, __LINE__);
	legacy_msm_jpeg_q_unblock(&pgmn_dev->output_rtn_q);
	return 0;
}
...
int legacy_msm_jpeg_output_buf_enqueue(struct msm_jpeg_device *pgmn_dev,
	void __user *arg)
{
	struct msm_jpeg_buf buf_cmd;
	struct msm_jpeg_core_buf *buf_p;
	...
	buf_p->y_buffer_addr = legacy_msm_jpeg_platform_v2p(pgmn_dev, buf_cmd.fd,
	...
	legacy_msm_jpeg_q_in(&pgmn_dev->output_buf_q, buf_p);
	return 0;
}

/*************** input queue ****************/

int legacy_msm_jpeg_fe_pingpong_irq(struct msm_jpeg_device *pgmn_dev,
	struct msm_jpeg_core_buf *buf_in)
{
	struct msm_jpeg_core_buf *buf_out;
	int rc = 0;
	...
	if (buf_in) {
		JPEG_DBG("%s:%d] 0x%08x %d\n", __func__, __LINE__,
			(int) buf_in->y_buffer_addr, buf_in->y_len);
		rc = legacy_msm_jpeg_q_in_buf(&pgmn_dev->input_rtn_q, buf_in);
	} else {
	...
	buf_out = legacy_msm_jpeg_q_out(&pgmn_dev->input_buf_q);

	if (buf_out) {
		rc = legacy_msm_jpeg_core_fe_buf_update(pgmn_dev, buf_out);
		kfree(buf_out);
		legacy_msm_jpeg_core_fe_start(pgmn_dev);
	} else {
	...
	if (buf_in)
		rc = legacy_msm_jpeg_q_wakeup(&pgmn_dev->input_rtn_q);

	return rc;
}

int legacy_msm_jpeg_input_get(struct msm_jpeg_device *pgmn_dev, void __user *to)
{
	struct msm_jpeg_core_buf *buf_p;
	struct msm_jpeg_buf buf_cmd;

	JPEG_DBG("%s:%d] Enter\n", __func__, __LINE__);
	legacy_msm_jpeg_q_wait(&pgmn_dev->input_rtn_q);
	buf_p = legacy_msm_jpeg_q_out(&pgmn_dev->input_rtn_q);
...
int legacy_msm_jpeg_input_get_unblock(struct msm_jpeg_device *pgmn_dev)
{
	JPEG_DBG("%s:%d] Enter\n", __func__, __LINE__);
	legacy_msm_jpeg_q_unblock(&pgmn_dev->input_rtn_q);
	return 0;
}

int legacy_msm_jpeg_input_buf_enqueue(struct msm_jpeg_device *pgmn_dev,
	void __user *arg)
{
	struct msm_jpeg_core_buf *buf_p;
	struct msm_jpeg_buf buf_cmd;
	...
	legacy_msm_jpeg_q_in(&pgmn_dev->input_buf_q, buf_p);

	return 0;
}

int legacy_msm_jpeg_irq(int event, void *context, void *data)
{
	...
	case MSM_JPEG_EVT_SESSION_DONE:
		legacy_msm_jpeg_framedone_irq(pgmn_dev, data);
		legacy_msm_jpeg_we_pingpong_irq(pgmn_dev, data);
		break;

	case MSM_JPEG_HW_MASK_COMP_FE:
		legacy_msm_jpeg_fe_pingpong_irq(pgmn_dev, data);
		break;
	...
	case MSM_JPEG_HW_MASK_COMP_RESET_ACK:
		legacy_msm_jpeg_reset_ack_irq(pgmn_dev);
		break;

	case MSM_JPEG_HW_MASK_COMP_ERR:
	default:
		legacy_msm_jpeg_err_irq(pgmn_dev, event);
		break;
	}
...
int legacy___msm_jpeg_open(struct msm_jpeg_device *pgmn_dev)
{
	...
	rc = legacy_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_JPEG,
			CAM_AHB_SVS_VOTE);
	if (rc < 0) {
	...
	legacy_msm_jpeg_core_irq_install(legacy_msm_jpeg_irq);
	if (pgmn_dev->core_type == MSM_JPEG_CORE_CODEC)
		core_irq = legacy_msm_jpeg_core_irq;
	else
		core_irq = legacy_msm_jpegdma_core_irq;

	/* initialize the platform resources */
	rc = legacy_msm_jpeg_platform_init(core_irq, pgmn_dev);
	if (rc) {
	...
	legacy_msm_jpeg_q_cleanup(&pgmn_dev->evt_q);
	legacy_msm_jpeg_q_cleanup(&pgmn_dev->output_rtn_q);
	legacy_msm_jpeg_outbuf_q_cleanup(pgmn_dev, &pgmn_dev->output_buf_q);
	legacy_msm_jpeg_q_cleanup(&pgmn_dev->input_rtn_q);
	legacy_msm_jpeg_q_cleanup(&pgmn_dev->input_buf_q);
	legacy_msm_jpeg_core_init(pgmn_dev);

	JPEG_DBG("%s:%d] success\n", __func__, __LINE__);
	return rc;

platform_init_fail:
	if (legacy_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_JPEG,
...
int legacy___msm_jpeg_release(struct msm_jpeg_device *pgmn_dev)
{
	...
	legacy_msm_jpeg_core_release(pgmn_dev);
	legacy_msm_jpeg_q_cleanup(&pgmn_dev->evt_q);
	legacy_msm_jpeg_q_cleanup(&pgmn_dev->output_rtn_q);
	legacy_msm_jpeg_outbuf_q_cleanup(pgmn_dev, &pgmn_dev->output_buf_q);
	legacy_msm_jpeg_q_cleanup(&pgmn_dev->input_rtn_q);
	legacy_msm_jpeg_outbuf_q_cleanup(pgmn_dev, &pgmn_dev->input_buf_q);
	...
	legacy_msm_jpeg_platform_release(pgmn_dev);
...
int legacy_msm_jpeg_ioctl_hw_cmd(struct msm_jpeg_device *pgmn_dev,
	void * __user arg)
{
	...
	is_copy_to_user = legacy_msm_jpeg_hw_exec_cmds(&hw_cmd, 1,
...
int legacy_msm_jpeg_ioctl_hw_cmds(struct msm_jpeg_device *pgmn_dev,
	void * __user arg)
{
	int is_copy_to_user;
	uint32_t len;
	...
	is_copy_to_user = legacy_msm_jpeg_hw_exec_cmds(hw_cmd_p, m,
		 pgmn_dev->res_size, pgmn_dev->base);
...
int legacy_msm_jpeg_start(struct msm_jpeg_device *pgmn_dev, void * __user arg,
	int (*hw_ioctl)(struct msm_jpeg_device *, void * __user))
{
	...
	legacy_msm_jpeg_platform_set_dt_config(pgmn_dev);

	/* update the bw with vector index "1" */
	legacy_msm_camera_update_bus_vector(pgmn_dev->bus_client, 1);
	...
	for (i = 0; i < 2; i++) {
		...
		if (buf_out) {
			legacy_msm_jpeg_core_fe_buf_update(pgmn_dev, buf_out);
			kfree(buf_out);
		} else {
			JPEG_DBG("%s:%d] no input buffer\n", __func__,
					__LINE__);
	...
	for (i = 0; i < 2; i++) {
		buf_out_free[i] = legacy_msm_jpeg_q_out(&pgmn_dev->output_buf_q);

		if (buf_out_free[i]) {
			legacy_msm_jpeg_core_we_buf_update(pgmn_dev, buf_out_free[i]);
...
int legacy_msm_jpeg_ioctl_reset(struct msm_jpeg_device *pgmn_dev, void * __user arg)
{
	int rc;
	...
	if (pgmn_dev->state == MSM_JPEG_INIT) {
		...
		rc = legacy_msm_jpeg_core_reset(pgmn_dev, pgmn_dev->op_mode,
...
int legacy_msm_jpeg_ioctl_test_dump_region(struct msm_jpeg_device *pgmn_dev,
	unsigned long arg)
{
	JPEG_DBG("%s:%d] Enter\n", __func__, __LINE__);
	legacy_msm_jpeg_io_dump(pgmn_dev->base, JPEG_REG_SIZE);
	return 0;
}

int legacy_msm_jpeg_ioctl_set_clk_rate(struct msm_jpeg_device *pgmn_dev,
	void * __user arg)
{
	long clk_rate;
	int rc;
	...
	rc = legacy_msm_jpeg_platform_set_clk_rate(pgmn_dev, clk_rate);
	if (rc < 0) {
...
#ifdef CONFIG_COMPAT
int legacy_msm_jpeg_get_ctrl_cmd32(struct msm_jpeg_ctrl_cmd *ctrl_cmd,
	void __user  *arg)
{
...
int legacy_msm_jpeg_put_ctrl_cmd32(struct msm_jpeg_ctrl_cmd *ctrl_cmd,
	void __user  *arg)
{
	struct msm_jpeg_ctrl_cmd32 ctrl_cmd32;
	unsigned long temp;
...
int legacy_msm_jpeg_get_jpeg_buf32(struct msm_jpeg_buf *jpeg_buf,
	void __user  *arg)
{
...
int legacy_msm_jpeg_put_jpeg_buf32(struct msm_jpeg_buf *jpeg_buf,
	void __user  *arg)
{
	struct msm_jpeg_buf32 jpeg_buf32;
	unsigned long temp;
...
int legacy_msm_jpeg_put_hw_cmd32(void __user *arg,
...
int legacy_msm_jpeg_get_hw_cmd32(struct msm_jpeg_hw_cmd *phw_cmd,
...
int legacy_msm_jpeg_ioctl_hw_cmds32(struct msm_jpeg_device *pgmn_dev,
	void __user *arg)
{
	int is_copy_to_user;
	uint32_t len, len32;
	...
	while (m--) {
		...
		legacy_msm_jpeg_get_hw_cmd32(dst, src, 0);
	}

	is_copy_to_user = legacy_msm_jpeg_hw_exec_cmds(phw_cmds->hw_cmd, phw_cmds->m,
			 pgmn_dev->res_size, pgmn_dev->base);
	...
	if (is_copy_to_user >= 0) {
		...
		while (m--) {
			...
			legacy_msm_jpeg_put_hw_cmd32(dst, src, 0);
		}
...
int legacy_msm_jpeg_ioctl_hw_cmd32(struct msm_jpeg_device *pgmn_dev,
		void * __user arg)
{
	struct msm_jpeg_hw_cmd hw_cmd;
	int is_copy_to_user;

	if (legacy_msm_jpeg_get_hw_cmd32(&hw_cmd, arg, 1)) {
		JPEG_PR_ERR("%s:%d] failed\n", __func__, __LINE__);
		return -EFAULT;
	}
	...
	if (is_copy_to_user >= 0) {
		if (legacy_msm_jpeg_put_hw_cmd32(arg, &hw_cmd, 1)) {
			JPEG_PR_ERR("%s:%d] failed\n", __func__, __LINE__);
			return -EFAULT;
		}
	} else
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case MSM_JPEG_IOCTL_GET_HW_VERSION:
		...
		rc = legacy_msm_jpeg_ioctl_hw_cmd(pgmn_dev, (void __user *) arg);
		break;
	case MSM_JPEG_IOCTL_GET_HW_VERSION32:
		JPEG_DBG("%s:%d] VERSION 1 32bit\n", __func__, __LINE__);
		rc = legacy_msm_jpeg_ioctl_hw_cmd32(pgmn_dev, (void __user *) arg);
		break;

	case MSM_JPEG_IOCTL_RESET:
		rc = legacy_msm_jpeg_ioctl_reset(pgmn_dev, (void __user *) arg);
		break;

	case MSM_JPEG_IOCTL_RESET32:
		rc = legacy_msm_jpeg_get_ctrl_cmd32(&ctrl_cmd,
			(void __user *) arg);
		if (rc < 0)
			break;

		set_fs(KERNEL_DS);
		rc = legacy_msm_jpeg_ioctl_reset(pgmn_dev, (void __user *) &ctrl_cmd);
		set_fs(old_fs);
		kfree(pctrl_cmd);
		break;

	case MSM_JPEG_IOCTL_STOP:
		rc = legacy_msm_jpeg_ioctl_hw_cmds(pgmn_dev, (void __user *) arg);
		pgmn_dev->state = MSM_JPEG_STOPPED;
		break;

	case MSM_JPEG_IOCTL_STOP32:
		rc = legacy_msm_jpeg_ioctl_hw_cmds32(pgmn_dev, (void __user *) arg);
		pgmn_dev->state = MSM_JPEG_STOPPED;
		break;

	case MSM_JPEG_IOCTL_START:
		rc = legacy_msm_jpeg_start(pgmn_dev, (void __user *) arg,
			legacy_msm_jpeg_ioctl_hw_cmds);
		break;

	case MSM_JPEG_IOCTL_START32:
		rc = legacy_msm_jpeg_start(pgmn_dev, (void __user *) arg,
				legacy_msm_jpeg_ioctl_hw_cmds32);
		break;

	case MSM_JPEG_IOCTL_INPUT_BUF_ENQUEUE:
		rc = legacy_msm_jpeg_input_buf_enqueue(pgmn_dev,
			(void __user *) arg);
		break;

	case MSM_JPEG_IOCTL_INPUT_BUF_ENQUEUE32:
		rc = legacy_msm_jpeg_get_jpeg_buf32(&jpeg_buf, (void __user *) arg);
		if (rc < 0)
			break;
		set_fs(KERNEL_DS);
		rc = legacy_msm_jpeg_input_buf_enqueue(pgmn_dev,
	...
	case MSM_JPEG_IOCTL_INPUT_GET:
		rc = legacy_msm_jpeg_input_get(pgmn_dev, (void __user *) arg);
		break;

	case MSM_JPEG_IOCTL_INPUT_GET32:
		set_fs(KERNEL_DS);
		rc = legacy_msm_jpeg_input_get(pgmn_dev, (void __user *) &jpeg_buf);
		set_fs(old_fs);
		if (rc < 0)
			break;
		rc = legacy_msm_jpeg_put_jpeg_buf32(&jpeg_buf, (void __user *) arg);

		break;

	case MSM_JPEG_IOCTL_INPUT_GET_UNBLOCK:
		rc = legacy_msm_jpeg_input_get_unblock(pgmn_dev);
		break;

	case MSM_JPEG_IOCTL_OUTPUT_BUF_ENQUEUE:
		rc = legacy_msm_jpeg_output_buf_enqueue(pgmn_dev,
			(void __user *) arg);
		break;
	...
	case MSM_JPEG_IOCTL_OUTPUT_GET:
		rc = legacy_msm_jpeg_output_get(pgmn_dev, (void __user *) arg);
		break;

	case MSM_JPEG_IOCTL_OUTPUT_GET32:
		set_fs(KERNEL_DS);
		rc = legacy_msm_jpeg_output_get(pgmn_dev, (void __user *) &jpeg_buf);
		set_fs(old_fs);
		if (rc < 0)
			break;
		rc = legacy_msm_jpeg_put_jpeg_buf32(&jpeg_buf, (void __user *) arg);
	...
	case MSM_JPEG_IOCTL_OUTPUT_GET_UNBLOCK:
		rc = legacy_msm_jpeg_output_get_unblock(pgmn_dev);
		break;

	case MSM_JPEG_IOCTL_EVT_GET:
		rc = legacy_msm_jpeg_evt_get(pgmn_dev, (void __user *) arg);
		break;

	case MSM_JPEG_IOCTL_EVT_GET32:
		set_fs(KERNEL_DS);
		rc = legacy_msm_jpeg_evt_get(pgmn_dev, (void __user *) &ctrl_cmd);
		set_fs(old_fs);
		if (rc < 0)
			break;
		legacy_msm_jpeg_put_ctrl_cmd32(&ctrl_cmd, (void __user *) arg);
		break;

	case MSM_JPEG_IOCTL_EVT_GET_UNBLOCK:
		rc = legacy_msm_jpeg_evt_get_unblock(pgmn_dev);
		break;
	...
	case MSM_JPEG_IOCTL_TEST_DUMP_REGION:
		rc = legacy_msm_jpeg_ioctl_test_dump_region(pgmn_dev, arg);
		break;
	...
	case MSM_JPEG_IOCTL_SET_CLK_RATE:
		rc = legacy_msm_jpeg_ioctl_set_clk_rate(pgmn_dev,
			(void __user *) arg);
		break;
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case MSM_JPEG_IOCTL_SET_CLK_RATE:
		rc = legacy_msm_jpeg_ioctl_set_clk_rate(pgmn_dev, (void __user *) arg);
		break;
	default:
...
int legacy___msm_jpeg_init(struct msm_jpeg_device *pgmn_dev)
{
	...
	legacy_msm_jpeg_q_init("evt_q", &pgmn_dev->evt_q);
	legacy_msm_jpeg_q_init("output_rtn_q", &pgmn_dev->output_rtn_q);
	legacy_msm_jpeg_q_init("output_buf_q", &pgmn_dev->output_buf_q);
	legacy_msm_jpeg_q_init("input_rtn_q", &pgmn_dev->input_rtn_q);
	legacy_msm_jpeg_q_init("input_buf_q", &pgmn_dev->input_buf_q);

	/*get device context for IOMMU*/
	rc = legacy_cam_smmu_get_handle(iommu_name[idx], &pgmn_dev->iommu_hdl);
	...
	rc = legacy_msm_jpeg_platform_setup(pgmn_dev);
	if (rc < 0) {
...
{
	legacy_msm_jpeg_platform_cleanup(pgmn_dev);
	mutex_destroy(&pgmn_dev->lock);
	kfree(pgmn_dev);
	return 0;
}
