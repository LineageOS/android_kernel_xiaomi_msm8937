...
void legacy_m_msm_vfe47_config_irq(struct vfe_device *vfe_dev,
		...
		enum msm_isp_irq_operation oper)
{
	...
	legacy_m_msm_camera_io_w_mb(vfe_dev->irq0_mask,
				vfe_dev->vfe_base + 0x5C);
	legacy_m_msm_camera_io_w_mb(vfe_dev->irq1_mask,
			...
			if (dt_parms->settings) {
				...
				if (rc < 0) {
					...
					for (i = 0; i < num_dt_entries; i++) {
						legacy_m_msm_camera_io_w(dt_settings[i],
							dev_mem_base +
								dt_regs[i]);
					}
					kfree(dt_settings);
...
int legacy_m_msm_vfe47_init_hardware(struct vfe_device *vfe_dev)
{
	int rc = -1;
	...
	rc = legacy_m_cam_config_ahb_clk(NULL, 0, id, CAM_AHB_SVS_VOTE);
	if (rc < 0) {
	...
	rc = legacy_m_msm_camera_enable_irq(vfe_dev->vfe_irq, 1);
	if (rc < 0)
		goto irq_enable_fail;

	return rc;
irq_enable_fail:
	legacy_m_msm_isp_update_bandwidth(ISP_VFE0 + vfe_dev->pdev->id, 0, 0);
bw_enable_fail:
	vfe_dev->common_data->dual_vfe_res->vfe_base[vfe_dev->pdev->id] = NULL;
	if (legacy_m_cam_config_ahb_clk(NULL, 0, id, CAM_AHB_SUSPEND_VOTE) < 0)
...
void legacy_m_msm_vfe47_release_hardware(struct vfe_device *vfe_dev)
{
	enum cam_ahb_clk_client id;
	...
	vfe_dev->hw_info->vfe_ops.irq_ops.config_irq(vfe_dev,
				...
				MSM_ISP_IRQ_SET);
	legacy_m_msm_camera_enable_irq(vfe_dev->vfe_irq, 0);
...
void legacy_m_msm_vfe47_init_hardware_reg(struct vfe_device *vfe_dev)
{
	...
	/* BUS_CFG */
	legacy_m_msm_camera_io_w(0x00000101, vfe_dev->vfe_base + 0x84);
	/* IRQ_MASK/CLEAR */
	legacy_m_msm_vfe47_config_irq(vfe_dev, 0x810000E0, 0xFFFFFF7E,
				MSM_ISP_IRQ_ENABLE);
	legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x64);
	legacy_m_msm_camera_io_w_mb(0xFFFFFFFF, vfe_dev->vfe_base + 0x68);
	legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x58);
}

void legacy_m_msm_vfe47_clear_status_reg(struct vfe_device *vfe_dev)
{
	legacy_m_msm_vfe47_config_irq(vfe_dev, 0x80000000, 0x0,
				MSM_ISP_IRQ_SET);
	legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x64);
	legacy_m_msm_camera_io_w_mb(0xFFFFFFFF, vfe_dev->vfe_base + 0x68);
	legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x58);
...
void legacy_m_msm_vfe47_process_reset_irq(struct vfe_device *vfe_dev,
...
void legacy_m_msm_vfe47_process_halt_irq(struct vfe_device *vfe_dev,
	uint32_t irq_status0, uint32_t irq_status1)
{
	uint32_t val = 0;
	...
	if (irq_status1 & (1 << 8)) {
		...
		legacy_m_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x400);
	}

	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_vbif_base + VFE47_VBIF_CLK_OFFSET);
	val &= ~(0x1);
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_vbif_base + VFE47_VBIF_CLK_OFFSET);
}

void legacy_m_msm_vfe47_process_input_irq(struct vfe_device *vfe_dev,
	uint32_t irq_status0, uint32_t irq_status1,
	struct msm_isp_timestamp *ts)
{
	...
	if (irq_status0 & (1 << 0)) {
		...
		legacy_m_msm_isp_increment_frame_id(vfe_dev, VFE_PIX_0, ts);
	}

	if (irq_status0 & (1 << 24)) {
		ISP_DBG("%s: Fetch Engine Read IRQ\n", __func__);
		legacy_m_msm_isp_fetch_engine_done_notify(vfe_dev,
...
void legacy_m_msm_vfe47_process_violation_status(
...
void legacy_m_msm_vfe47_process_error_status(struct vfe_device *vfe_dev)
{
	...
	if (error_status1 & (1 << 0)) {
		...
		legacy_m_msm_camera_io_dump(vfe_dev->vfe_base + 0x478, 0x3C, 1);
		/* testgen */
		if (TESTGEN == vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux)
			legacy_m_msm_camera_io_dump(vfe_dev->vfe_base + 0xC58, 0x28, 1);
	}
	...
	if (error_status1 & (1 << 7)) {
		legacy_m_msm_vfe47_process_violation_status(vfe_dev);
	}
...
void legacy_m_msm_vfe47_read_irq_status(struct vfe_device *vfe_dev,
	uint32_t *irq_status0, uint32_t *irq_status1)
{
	uint32_t count = 0;
	*irq_status0 = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x6C);
	*irq_status1 = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x70);
	/* Mask off bits that are not enabled */
	legacy_m_msm_camera_io_w(*irq_status0, vfe_dev->vfe_base + 0x64);
	legacy_m_msm_camera_io_w(*irq_status1, vfe_dev->vfe_base + 0x68);
	...
	*irq_status1 &= vfe_dev->irq1_mask;
	...
	if (*irq_status1 & (1 << 0)) {
		...
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x4A4);
		/* mask off camif error after first occurrance */
		legacy_m_msm_vfe47_config_irq(vfe_dev, 0, (1 << 0), MSM_ISP_IRQ_DISABLE);
	}

	if (*irq_status1 & (1 << 7))
		vfe_dev->error_info.violation_status =
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x7C);

}

void legacy_m_msm_vfe47_process_reg_update(struct vfe_device *vfe_dev,
	uint32_t irq_status0, uint32_t irq_status1,
	struct msm_isp_timestamp *ts)
{
	...
	for (i = VFE_PIX_0; i <= VFE_RAW_2; i++) {
		if (shift_irq & BIT(i)) {
			...
			switch (i) {
			case VFE_PIX_0:
				legacy_m_msm_isp_notify(vfe_dev, ISP_EVENT_REG_UPDATE,
					VFE_PIX_0, ts);
				if (atomic_read(
					&vfe_dev->stats_data.stats_update))
					legacy_m_msm_isp_stats_stream_update(vfe_dev);
			...
			case VFE_RAW_1:
			case VFE_RAW_2:
				legacy_m_msm_isp_increment_frame_id(vfe_dev, i, ts);
				legacy_m_msm_isp_notify(vfe_dev, ISP_EVENT_SOF, i, ts);
				legacy_m_msm_isp_update_framedrop_reg(vfe_dev, i);
				/*
			...
			if (vfe_dev->axi_data.stream_update[i])
				legacy_m_msm_isp_axi_stream_update(vfe_dev, i);
			legacy_m_msm_isp_save_framedrop_values(vfe_dev, i);
			if (atomic_read(&vfe_dev->axi_data.axi_cfg_update[i])) {
				legacy_m_msm_isp_axi_cfg_update(vfe_dev, i);
				if (atomic_read(
					&vfe_dev->axi_data.axi_cfg_update[i]) ==
					0)
					legacy_m_msm_isp_notify(vfe_dev,
...
void legacy_m_msm_vfe47_process_epoch_irq(struct vfe_device *vfe_dev,
	uint32_t irq_status0, uint32_t irq_status1,
	struct msm_isp_timestamp *ts)
{
	if (!(irq_status0 & 0xc))
	...
	if (irq_status0 & BIT(2)) {
		...
		legacy_m_msm_isp_update_framedrop_reg(vfe_dev, VFE_PIX_0);
		legacy_m_msm_isp_update_stats_framedrop_reg(vfe_dev);
		legacy_m_msm_isp_update_error_frame_count(vfe_dev);
		legacy_m_msm_isp_notify(vfe_dev, ISP_EVENT_SOF, VFE_PIX_0, ts);
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].raw_stream_count > 0
			&& vfe_dev->axi_data.src_info[VFE_PIX_0].
			pix_stream_count == 0) {
			if (vfe_dev->axi_data.stream_update[VFE_PIX_0])
				legacy_m_msm_isp_axi_stream_update(vfe_dev, VFE_PIX_0);
...
void legacy_m_msm_vfe47_reg_update(struct vfe_device *vfe_dev,
	enum msm_vfe_input_src frame_src)
{
	...
	if ((vfe_dev->is_split && vfe_dev->pdev->id == ISP_VFE1) &&
		...
		legacy_m_msm_camera_io_w_mb(update_mask,
...
long legacy_m_msm_vfe47_reset_hardware(struct vfe_device *vfe_dev,
	uint32_t first_start, uint32_t blocking_call)
{
	...
	if (first_start) {
		if (msm_vfe_is_vfe48(vfe_dev))
			...
			reset = 0x3FF;
		legacy_m_msm_camera_io_w_mb(reset, vfe_dev->vfe_base + 0x18);
	} else {
		if (msm_vfe_is_vfe48(vfe_dev))
			reset = 0x3E7;
		else
			reset = 0x3EF;
		legacy_m_msm_camera_io_w_mb(reset, vfe_dev->vfe_base + 0x18);
		legacy_m_msm_camera_io_w(0x7FFFFFFF, vfe_dev->vfe_base + 0x64);
		legacy_m_msm_camera_io_w(0xFFFFFEFF, vfe_dev->vfe_base + 0x68);
		legacy_m_msm_camera_io_w(0x1, vfe_dev->vfe_base + 0x58);
...
void legacy_m_msm_vfe47_axi_reload_wm(struct vfe_device *vfe_dev,
	void __iomem *vfe_base, uint32_t reload_mask)
{
	legacy_m_msm_camera_io_w_mb(reload_mask, vfe_base + 0x80);
}

void legacy_m_msm_vfe47_axi_update_cgc_override(struct vfe_device *vfe_dev,
	uint8_t wm_idx, uint8_t enable)
{
	uint32_t val;

	/* Change CGC override */
	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x3C);
	if (enable)
		val |= (1 << wm_idx);
	else
		val &= ~(1 << wm_idx);
	legacy_m_msm_camera_io_w_mb(val, vfe_dev->vfe_base + 0x3C);
}
	...
	uint8_t wm_idx, uint8_t enable)
{
	...
	val = legacy_m_msm_camera_io_r(vfe_base + VFE47_WM_BASE(wm_idx));
	if (enable)
		val |= 0x1;
	else
		val &= ~0x1;
...
void legacy_m_msm_vfe47_axi_cfg_comp_mask(struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info)
{
	...
	comp_mask = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x74);
	comp_mask &= ~(0x7F << (comp_mask_index * 8));
	comp_mask |= (axi_data->composite_info[comp_mask_index].
		stream_composite_mask << (comp_mask_index * 8));
	legacy_m_msm_camera_io_w(comp_mask, vfe_dev->vfe_base + 0x74);

	legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << (comp_mask_index + 25), 0,
				MSM_ISP_IRQ_ENABLE);
}

void legacy_m_msm_vfe47_axi_clear_comp_mask(struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info)
{
...
void legacy_m_msm_vfe47_axi_cfg_wm_irq_mask(struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info)
{
	legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << (stream_info->wm[0] + 8), 0,
				MSM_ISP_IRQ_ENABLE);
}

void legacy_m_msm_vfe47_axi_clear_wm_irq_mask(struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info)
{
	legacy_m_msm_vfe47_config_irq(vfe_dev, (1 << (stream_info->wm[0] + 8)), 0,
				MSM_ISP_IRQ_DISABLE);
...
void legacy_m_msm_vfe47_cfg_framedrop(void __iomem *vfe_base,
	...
	uint32_t framedrop_period)
{
	...
	for (i = 0; i < stream_info->num_planes; i++) {
		legacy_m_msm_camera_io_w(framedrop_pattern, vfe_base +
			VFE47_WM_BASE(stream_info->wm[i]) + 0x24);
		temp = legacy_m_msm_camera_io_r(vfe_base +
			VFE47_WM_BASE(stream_info->wm[i]) + 0x14);
		temp &= 0xFFFFFF83;
		legacy_m_msm_camera_io_w(temp | (framedrop_period - 1) << 2,
		vfe_base + VFE47_WM_BASE(stream_info->wm[i]) + 0x14);
	}
}

void legacy_m_msm_vfe47_clear_framedrop(struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info)
{
	uint32_t i;

	for (i = 0; i < stream_info->num_planes; i++)
		legacy_m_msm_camera_io_w(0, vfe_dev->vfe_base +
			VFE47_WM_BASE(stream_info->wm[i]) + 0x24);
}
...
int32_t legacy_m_msm_vfe47_cfg_io_format(struct vfe_device *vfe_dev,
	enum msm_vfe_axi_stream_src stream_src, uint32_t io_format)
{
	...
	io_format_reg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x88);

	/*input config*/
	if ((stream_src < RDI_INTF_0) &&
		(vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux ==
		EXTERNAL_READ)) {
		read_bpp = legacy_m_msm_isp_get_bit_per_pixel(
		...
		read_pack_fmt = legacy_m_msm_isp_get_pack_format(
			vfe_dev->axi_data.src_info[VFE_PIX_0].input_format);
	...
	bpp = legacy_m_msm_isp_get_bit_per_pixel(io_format);
	...
	case IDEAL_RAW:
		...
		pack_fmt = legacy_m_msm_isp_get_pack_format(io_format);
	...
	legacy_m_msm_camera_io_w(io_format_reg, vfe_dev->vfe_base + 0x88);
	return 0;
}

int legacy_m_msm_vfe47_start_fetch_engine(struct vfe_device *vfe_dev,
	void *arg)
{
	int rc = 0;
	...
	legacy_m_msm_camera_io_w(mapped_info.paddr, vfe_dev->vfe_base + 0x2F4);

	legacy_m_msm_camera_io_w_mb(0x100000, vfe_dev->vfe_base + 0x80);
	legacy_m_msm_camera_io_w_mb(0x200000, vfe_dev->vfe_base + 0x80);
...
int legacy_m_msm_vfe47_start_fetch_engine_multi_pass(struct vfe_device *vfe_dev,
	void *arg)
{
	int rc = 0;
	uint32_t bufq_handle = 0;
	...
	legacy_m_msm_camera_io_w(mapped_info.paddr + fe_cfg->input_buf_offset,
...
void legacy_m_msm_vfe47_cfg_fetch_engine(struct vfe_device *vfe_dev,
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	if (pix_cfg->input_mux == EXTERNAL_READ) {
		...
		temp = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x84);
		temp &= 0xFFFFFFFD;
		temp |= (1 << 1);
		legacy_m_msm_camera_io_w(temp, vfe_dev->vfe_base + 0x84);

		legacy_m_msm_vfe47_config_irq(vfe_dev, (1 << 24), 0,
				MSM_ISP_IRQ_ENABLE);

		temp = fe_cfg->fetch_height - 1;
		legacy_m_msm_camera_io_w(temp & 0x3FFF, vfe_dev->vfe_base + 0x308);

		x_size_word = legacy_m_msm_isp_cal_word_per_line(
			vfe_dev->axi_data.src_info[VFE_PIX_0].input_format,
			fe_cfg->buf_width);
		legacy_m_msm_camera_io_w((x_size_word - 1) << 16,
			vfe_dev->vfe_base + 0x30c);

		x_size_word = legacy_m_msm_isp_cal_word_per_line(
			vfe_dev->axi_data.src_info[VFE_PIX_0].input_format,
			fe_cfg->fetch_width);
		legacy_m_msm_camera_io_w(x_size_word << 16 |
			(temp & 0x3FFF) << 2 | VFE47_FETCH_BURST_LEN,
			vfe_dev->vfe_base + 0x310);

		temp = ((fe_cfg->buf_width - 1) & 0x3FFF) << 16 |
			((fe_cfg->buf_height - 1) & 0x3FFF);
		legacy_m_msm_camera_io_w(temp, vfe_dev->vfe_base + 0x314);

		/* need to use formulae to calculate MAIN_UNPACK_PATTERN*/
		legacy_m_msm_camera_io_w(0xF6543210, vfe_dev->vfe_base + 0x318);
		legacy_m_msm_camera_io_w(0xF, vfe_dev->vfe_base + 0x334);

		temp = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x50);
		temp |= 2 << 5;
		temp |= 128 << 8;
		temp |= (pix_cfg->pixel_pattern & 0x3);
		legacy_m_msm_camera_io_w(temp, vfe_dev->vfe_base + 0x50);
...
void legacy_m_msm_vfe47_cfg_testgen(struct vfe_device *vfe_dev,
	struct msm_vfe_testgen_cfg *testgen_cfg)
{
	...
	bit_per_pixel = legacy_m_msm_isp_get_bit_per_pixel(
		vfe_dev->axi_data.src_info[VFE_PIX_0].input_format);
	...
	legacy_m_msm_camera_io_w(bpp_reg << 16 | testgen_cfg->burst_num_frame,
		vfe_dev->vfe_base + 0xC5C);

	legacy_m_msm_camera_io_w(((testgen_cfg->lines_per_frame - 1) << 16) |
	...
	legacy_m_msm_camera_io_w((1 << 16) | testgen_cfg->v_blank,
	...
	legacy_m_msm_camera_io_w((testgen_cfg->rotate_period << 8) |
		(bayer_pix_pattern_reg << 6) | (unicolor_enb << 4) |
		(unicolorbar_reg), vfe_dev->vfe_base + 0xC78);
	return;
}

void legacy_m_msm_vfe47_cfg_camif(struct vfe_device *vfe_dev,
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	legacy_m_msm_camera_io_w(pix_cfg->input_mux << 5 | pix_cfg->pixel_pattern,
	...
	legacy_m_msm_camera_io_w((camif_cfg->lines_per_frame - 1) << 16 |
		(camif_cfg->pixels_per_line - 1), vfe_dev->vfe_base + 0x484);
	if (bus_sub_en) {
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x47C);
		val &= 0xFFFFFFDF;
		val = val | bus_sub_en << 5;
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x47C);
		subsample_cfg->pixel_skip &= 0x0000FFFF;
		subsample_cfg->line_skip  &= 0x0000FFFF;
		legacy_m_msm_camera_io_w((subsample_cfg->line_skip << 16) |
			subsample_cfg->pixel_skip, vfe_dev->vfe_base + 0x490);
	}


	legacy_m_msm_camera_io_w(first_pixel << 16 | last_pixel,
	vfe_dev->vfe_base + 0x488);

	legacy_m_msm_camera_io_w(first_line << 16 | last_line,
	vfe_dev->vfe_base + 0x48C);

	legacy_m_msm_camera_io_w(((irq_sub_period - 1) << 8) | 0 << 5 |
		(frame_sub_period - 1), vfe_dev->vfe_base + 0x494);
	legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x498);
	legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x49C);
	if (subsample_period && subsample_pattern) {
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x494);
		val &= 0xFFFFE0FF;
		val = (subsample_period - 1) << 8;
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x494);
		ISP_DBG("%s:camif PERIOD %x PATTERN %x\n",
			__func__,  subsample_period, subsample_pattern);

		val = subsample_pattern;
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x49C);
	} else {
		legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x49C);
	}
	...
	if (subsample_cfg->first_pixel ||
		...
		subsample_cfg->first_line << 16 |
			subsample_cfg->last_line,
			vfe_dev->vfe_base + 0xCE4);
		val = legacy_m_msm_camera_io_r(
			vfe_dev->vfe_base + 0x47C);
		ISP_DBG("%s: camif raw crop enabled\n", __func__);
		val |= 1 << 22;
		legacy_m_msm_camera_io_w(val,
			vfe_dev->vfe_base + 0x47C);
	}
	...
	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x88);
	...
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x88);

	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x46C);
	val |= camif_cfg->camif_input;
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x46C);
}

void legacy_m_msm_vfe47_cfg_input_mux(struct vfe_device *vfe_dev,
	struct msm_vfe_pix_cfg *pix_cfg)
{
	uint32_t core_cfg = 0;
	uint32_t val = 0;

	core_cfg =  legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x50);
	core_cfg &= 0xFFFFFF9F;

	switch (pix_cfg->input_mux) {
	case CAMIF:
		core_cfg |= 0x0 << 5;
		legacy_m_msm_camera_io_w_mb(core_cfg, vfe_dev->vfe_base + 0x50);
		legacy_m_msm_vfe47_cfg_camif(vfe_dev, pix_cfg);
		break;
	case TESTGEN:
		/* Change CGC override */
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x3C);
		val |= (1 << 31);
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x3C);

		/* CAMIF and TESTGEN will both go thorugh CAMIF*/
		core_cfg |= 0x1 << 5;
		legacy_m_msm_camera_io_w_mb(core_cfg, vfe_dev->vfe_base + 0x50);
		legacy_m_msm_vfe47_cfg_camif(vfe_dev, pix_cfg);
		legacy_m_msm_vfe47_cfg_testgen(vfe_dev, &pix_cfg->testgen_cfg);
		break;
	case EXTERNAL_READ:
		core_cfg |= 0x2 << 5;
		legacy_m_msm_camera_io_w_mb(core_cfg, vfe_dev->vfe_base + 0x50);
		legacy_m_msm_vfe47_cfg_fetch_engine(vfe_dev, pix_cfg);
		break;
	default:
...
void legacy_m_msm_vfe47_configure_hvx(struct vfe_device *vfe_dev,
	uint8_t is_stream_on)
{
	uint32_t val;
	if (is_stream_on == 1) {
		/* Enable HVX */
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x50);
		val |= (1 << 3);
		legacy_m_msm_camera_io_w_mb(val, vfe_dev->vfe_base + 0x50);
		val &= 0xFF7FFFFF;
...
void legacy_m_msm_vfe47_update_camif_state(struct vfe_device *vfe_dev,
	enum msm_isp_camif_update_state update_state)
{
	...
	if (update_state == ENABLE_CAMIF) {
		legacy_m_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x64);
		legacy_m_msm_camera_io_w(0x81, vfe_dev->vfe_base + 0x68);
		legacy_m_msm_camera_io_w(0x1, vfe_dev->vfe_base + 0x58);
		legacy_m_msm_vfe47_config_irq(vfe_dev, 0x15, 0x81,
					MSM_ISP_IRQ_ENABLE);

		if ((vfe_dev->hvx_cmd > HVX_DISABLE) &&
			(vfe_dev->hvx_cmd <= HVX_ROUND_TRIP))
			legacy_m_msm_vfe47_configure_hvx(vfe_dev, 1);
		else
			legacy_m_msm_vfe47_configure_hvx(vfe_dev, 0);
		...
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x47C);
		legacy_m_msm_camera_io_w_mb(0x4, vfe_dev->vfe_base + 0x478);
		legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x478);
		/* configure EPOCH0 for 20 lines */
		legacy_m_msm_camera_io_w_mb(0x140000, vfe_dev->vfe_base + 0x4A0);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 1;
		/* testgen GO*/
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			legacy_m_msm_camera_io_w(1, vfe_dev->vfe_base + 0xC58);
	} else if (update_state == DISABLE_CAMIF ||
		update_state == DISABLE_CAMIF_IMMEDIATELY) {
		/* turn off camif violation and error irqs */
		legacy_m_msm_vfe47_config_irq(vfe_dev, 0, 0x81,
					MSM_ISP_IRQ_DISABLE);
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x464);
		/* disable danger signal */
		legacy_m_msm_camera_io_w_mb(val & ~(1 << 8), vfe_dev->vfe_base + 0x464);
		legacy_m_msm_camera_io_w_mb((update_state == DISABLE_CAMIF ? 0x0 : 0x6),
				vfe_dev->vfe_base + 0x478);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 0;
		/* testgen OFF*/
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			legacy_m_msm_camera_io_w(1 << 1, vfe_dev->vfe_base + 0xC58);
...
void legacy_m_msm_vfe47_cfg_rdi_reg(
	...
	enum msm_vfe_input_src input_src)
{
	...
	rdi_reg_cfg = legacy_m_msm_camera_io_r(
...
void legacy_m_msm_vfe47_axi_cfg_wm_reg(
	struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info,
	uint8_t plane_idx)
{
	...
	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + wm_base + 0x14);
	val &= ~0x2;
	if (stream_info->frame_based)
		val |= 0x2;
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x14);
	if (!stream_info->frame_based) {
		/* WR_IMAGE_SIZE */
		val = ((legacy_m_msm_isp_cal_word_per_line(
			stream_info->output_format,
			stream_info->plane_cfg[plane_idx].
			output_width)+3)/4 - 1) << 16 |
			(stream_info->plane_cfg[plane_idx].
			output_height - 1);
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x1C);
		/* WR_BUFFER_CFG */
		val = VFE47_BURST_LEN |
			(stream_info->plane_cfg[plane_idx].output_height - 1) <<
			2 |
			((legacy_m_msm_isp_cal_word_per_line(stream_info->output_format,
			stream_info->plane_cfg[plane_idx].
			output_stride)+1)/2) << 16;
	}
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x20);
	/* WR_IRQ_SUBSAMPLE_PATTERN */
	legacy_m_msm_camera_io_w(0xFFFFFFFF,
		vfe_dev->vfe_base + wm_base + 0x28);
}

void legacy_m_msm_vfe47_axi_clear_wm_reg(
	struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info, uint8_t plane_idx)
{
	...
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x20);
	/* WR_IRQ_SUBSAMPLE_PATTERN */
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x28);
}

void legacy_m_msm_vfe47_axi_cfg_wm_xbar_reg(
	struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info,
	uint8_t plane_idx)
{
	...
	xbar_reg_cfg =
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + VFE47_XBAR_BASE(wm));
	xbar_reg_cfg &= ~(0xFFFF << VFE47_XBAR_SHIFT(wm));
	xbar_reg_cfg |= (xbar_cfg << VFE47_XBAR_SHIFT(wm));
	legacy_m_msm_camera_io_w(xbar_reg_cfg,
		vfe_dev->vfe_base + VFE47_XBAR_BASE(wm));
}

void legacy_m_msm_vfe47_axi_clear_wm_xbar_reg(
	struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info, uint8_t plane_idx)
{
	uint8_t wm = stream_info->wm[plane_idx];
...
void legacy_m_msm_vfe47_cfg_axi_ub_equal_default(
	struct vfe_device *vfe_dev)
{
	int i;
	...
	for (i = 0; i < axi_data->hw_info->num_wm; i++) {
		if (axi_data->free_wm[i]) {
			...
			wm_ub_size = axi_data->hw_info->min_wm_ub +
					(uint32_t)delta;
			legacy_m_msm_camera_io_w(ub_offset << 16 | (wm_ub_size - 1),
				vfe_dev->vfe_base + VFE47_WM_BASE(i) + 0x18);
			ub_offset += wm_ub_size;
		} else
			legacy_m_msm_camera_io_w(0,
				vfe_dev->vfe_base + VFE47_WM_BASE(i) + 0x18);
	}
}

void legacy_m_msm_vfe47_cfg_axi_ub_equal_slicing(
	struct vfe_device *vfe_dev)
{
	int i;
	uint32_t ub_offset = 0;
	...
	for (i = 0; i < axi_data->hw_info->num_wm; i++) {
		legacy_m_msm_camera_io_w(ub_offset << 16 | (ub_equal_slice - 1),
			vfe_dev->vfe_base + VFE47_WM_BASE(i) + 0x18);
		ub_offset += ub_equal_slice;
	}
}

void legacy_m_msm_vfe47_cfg_axi_ub(struct vfe_device *vfe_dev)
{
	struct msm_vfe_axi_shared_data *axi_data = &vfe_dev->axi_data;

	axi_data->wm_ub_cfg_policy = UB_CFG_POLICY;
	if (axi_data->wm_ub_cfg_policy == MSM_WM_UB_EQUAL_SLICING)
		legacy_m_msm_vfe47_cfg_axi_ub_equal_slicing(vfe_dev);
	else
		legacy_m_msm_vfe47_cfg_axi_ub_equal_default(vfe_dev);
}

void legacy_m_msm_vfe47_read_wm_ping_pong_addr(
	struct vfe_device *vfe_dev)
{
...
void legacy_m_msm_vfe47_update_ping_pong_addr(
	...
	int32_t buf_size)
{
	...
	legacy_m_msm_camera_io_w(paddr32, vfe_base +
		VFE47_PING_PONG_BASE(wm_idx, pingpong_bit));
	legacy_m_msm_camera_io_w(paddr32_max, vfe_base +
...
{
	legacy_m_msm_vfe47_config_irq(vfe_dev, BIT(31), BIT(8), MSM_ISP_IRQ_SET);
}

int legacy_m_msm_vfe47_axi_halt(struct vfe_device *vfe_dev,
	uint32_t blocking)
{
	int rc = 0;
	enum msm_vfe_input_src i;
	...
	if (blocking) {
		...
		/* Halt AXI Bus Bridge */
		legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x400);
		rc = wait_for_completion_timeout(
...
int legacy_m_msm_vfe47_axi_restart(struct vfe_device *vfe_dev,
	uint32_t blocking, uint32_t enable_camif)
{
	...
	legacy_m_msm_vfe47_config_irq(vfe_dev, vfe_dev->recovery_irq0_mask,
...
uint32_t legacy_m_msm_vfe47_get_wm_mask(
	uint32_t irq_status0, uint32_t irq_status1)
{
	return (irq_status0 >> 8) & 0x7F;
}

uint32_t legacy_m_msm_vfe47_get_comp_mask(
	uint32_t irq_status0, uint32_t irq_status1)
{
	return (irq_status0 >> 25) & 0xF;
}

uint32_t legacy_m_msm_vfe47_get_pingpong_status(
	struct vfe_device *vfe_dev)
{
	return legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x338);
}

int legacy_m_msm_vfe47_get_stats_idx(enum msm_isp_stats_type stats_type)
{
...
int legacy_m_msm_vfe47_stats_check_streams(
	struct msm_vfe_stats_stream *stream_info)
{
	return 0;
}

void legacy_m_msm_vfe47_stats_cfg_comp_mask(
	...
	uint8_t request_comp_index, uint8_t enable)
{
	...
	comp_mask_reg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x78);

	if (enable) {
		comp_mask_reg |= stats_mask << (request_comp_index * 16);
		atomic_set(stats_comp_mask, stats_mask |
				atomic_read(stats_comp_mask));
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << (29 + request_comp_index),
	...
	legacy_m_msm_camera_io_w(comp_mask_reg, vfe_dev->vfe_base + 0x78);
...
void legacy_m_msm_vfe47_stats_cfg_wm_irq_mask(
	struct vfe_device *vfe_dev,
	struct msm_vfe_stats_stream *stream_info)
{
	switch (STATS_IDX(stream_info->stream_handle)) {
	case STATS_COMP_IDX_AEC_BG:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 15, 0, MSM_ISP_IRQ_ENABLE);
		break;
	case STATS_COMP_IDX_HDR_BE:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 16, 0, MSM_ISP_IRQ_ENABLE);
		break;
	case STATS_COMP_IDX_BG:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 17, 0, MSM_ISP_IRQ_ENABLE);
		break;
	case STATS_COMP_IDX_BF:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 18, 1 << 26,
				MSM_ISP_IRQ_ENABLE);
		break;
	case STATS_COMP_IDX_HDR_BHIST:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 19, 0, MSM_ISP_IRQ_ENABLE);
		break;
	case STATS_COMP_IDX_RS:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 20, 0, MSM_ISP_IRQ_ENABLE);
		break;
	case STATS_COMP_IDX_CS:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 21, 0, MSM_ISP_IRQ_ENABLE);
		break;
	case STATS_COMP_IDX_IHIST:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 22, 0, MSM_ISP_IRQ_ENABLE);
		break;
	case STATS_COMP_IDX_BHIST:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 23, 0, MSM_ISP_IRQ_ENABLE);
		break;
	default:
...
void legacy_m_msm_vfe47_stats_clear_wm_irq_mask(
	struct vfe_device *vfe_dev,
	struct msm_vfe_stats_stream *stream_info)
{
	...
	switch (STATS_IDX(stream_info->stream_handle)) {
	case STATS_COMP_IDX_AEC_BG:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 15, 0, MSM_ISP_IRQ_DISABLE);
		break;
	case STATS_COMP_IDX_HDR_BE:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 16, 0, MSM_ISP_IRQ_DISABLE);
		break;
	case STATS_COMP_IDX_BG:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 17, 0, MSM_ISP_IRQ_DISABLE);
		break;
	case STATS_COMP_IDX_BF:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 18, 1 << 26,
				MSM_ISP_IRQ_DISABLE);
		break;
	case STATS_COMP_IDX_HDR_BHIST:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 19, 0, MSM_ISP_IRQ_DISABLE);
		break;
	case STATS_COMP_IDX_RS:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 20, 0, MSM_ISP_IRQ_DISABLE);
		break;
	case STATS_COMP_IDX_CS:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 21, 0, MSM_ISP_IRQ_DISABLE);
		break;
	case STATS_COMP_IDX_IHIST:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 22, 0, MSM_ISP_IRQ_DISABLE);
		break;
	case STATS_COMP_IDX_BHIST:
		legacy_m_msm_vfe47_config_irq(vfe_dev, 1 << 23, 0, MSM_ISP_IRQ_DISABLE);
		break;
	default:
		pr_err("%s: Invalid stats idx %d\n", __func__,
			STATS_IDX(stream_info->stream_handle));
...
void legacy_m_msm_vfe47_stats_cfg_wm_reg(
	struct vfe_device *vfe_dev,
	struct msm_vfe_stats_stream *stream_info)
{
	...
	/* WR_ADDR_CFG */
	legacy_m_msm_camera_io_w(stream_info->framedrop_period << 2,
		vfe_dev->vfe_base + stats_base + 0x10);
	/* WR_IRQ_FRAMEDROP_PATTERN */
	legacy_m_msm_camera_io_w(stream_info->framedrop_pattern,
...
void legacy_m_msm_vfe47_stats_clear_wm_reg(
	struct vfe_device *vfe_dev,
	struct msm_vfe_stats_stream *stream_info)
{
	uint32_t val = 0;
	...
	/* WR_ADDR_CFG */
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + stats_base + 0x10);
	/* WR_IRQ_FRAMEDROP_PATTERN */
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + stats_base + 0x18);
	/* WR_IRQ_SUBSAMPLE_PATTERN */
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + stats_base + 0x1C);
}

void legacy_m_msm_vfe47_stats_cfg_ub(struct vfe_device *vfe_dev)
{
	int i;
	uint32_t ub_offset = 0;
	...
	for (i = 0; i < VFE47_NUM_STATS_TYPE; i++) {
		ub_offset -= ub_size[i];
		legacy_m_msm_camera_io_w(VFE47_STATS_BURST_LEN << 30 |
			ub_offset << 16 | (ub_size[i] - 1),
			vfe_dev->vfe_base + VFE47_STATS_BASE(i) + 0x14);
	}
}

void legacy_m_msm_vfe47_stats_update_cgc_override(struct vfe_device *vfe_dev,
	uint32_t stats_mask, uint8_t enable)
{
	...
	module_cfg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x30);
	if (enable)
		module_cfg |= cgc_mask;
	else
		module_cfg &= ~cgc_mask;
	legacy_m_msm_camera_io_w(module_cfg, vfe_dev->vfe_base + 0x30);
}

bool legacy_m_msm_vfe47_is_module_cfg_lock_needed(
	uint32_t reg_offset)
{
	return false;
}

void legacy_m_msm_vfe47_stats_enable_module(struct vfe_device *vfe_dev,
	uint32_t stats_mask, uint8_t enable)
{
	int i;
	...
	module_cfg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x44);
	if (enable)
		module_cfg |= module_cfg_mask;
	else
		module_cfg &= ~module_cfg_mask;

	legacy_m_msm_camera_io_w(module_cfg, vfe_dev->vfe_base + 0x44);
	/* enable wm if needed */
...
void legacy_m_msm_vfe47_stats_update_ping_pong_addr(
...
uint32_t legacy_m_msm_vfe47_stats_get_wm_mask(
	uint32_t irq_status0, uint32_t irq_status1)
{
...
uint32_t legacy_m_msm_vfe47_stats_get_comp_mask(
	uint32_t irq_status0, uint32_t irq_status1)
{
	return (irq_status0 >> 29) & 0x3;
}

uint32_t legacy_m_msm_vfe47_stats_get_frame_id(
	struct vfe_device *vfe_dev)
{
	return vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id;
}

void legacy_m_msm_vfe47_deinit_bandwidth_mgr(
...
int legacy_m_msm_vfe47_init_bandwidth_mgr(struct vfe_device *vfe_dev,
	struct msm_isp_bandwidth_mgr *isp_bandwidth_mgr)
{
...
int legacy_m_msm_vfe47_update_bandwidth(
		struct msm_isp_bandwidth_mgr *isp_bandwidth_mgr)
{
	int i;
	uint64_t ab = 0;
...
int legacy_m_msm_vfe47_get_clks(struct vfe_device *vfe_dev)
{
	int i, rc;

	rc = legacy_m_msm_camera_get_clk_info(vfe_dev->pdev, &vfe_dev->vfe_clk_info,
...
void legacy_m_msm_vfe47_put_clks(struct vfe_device *vfe_dev)
{
	legacy_m_msm_camera_put_clk_info(vfe_dev->pdev, &vfe_dev->vfe_clk_info,
			&vfe_dev->vfe_clk, vfe_dev->num_clk);

	vfe_dev->num_clk = 0;
}

int legacy_m_msm_vfe47_enable_clks(struct vfe_device *vfe_dev, int enable)
{
	return legacy_m_msm_camera_clk_enable(&vfe_dev->pdev->dev,
			vfe_dev->vfe_clk_info,
			vfe_dev->vfe_clk, vfe_dev->num_clk, enable);
}

int legacy_m_msm_vfe47_set_clk_rate(struct vfe_device *vfe_dev, long *rate)
{
	int rc = 0;
	int clk_idx = vfe_dev->hw_info->vfe_clk_idx;

	rc = legacy_m_msm_camera_clk_set_rate(&vfe_dev->pdev->dev,
...
int legacy_m_msm_vfe47_get_max_clk_rate(struct vfe_device *vfe_dev, long *rate)
{
	int	   clk_idx = 0;
...
int legacy_m_msm_vfe47_get_clk_rates(struct vfe_device *vfe_dev,
...
void legacy_m_msm_vfe47_put_regulators(struct vfe_device *vfe_dev)
{
	int i;
...
int legacy_m_msm_vfe47_get_regulators(struct vfe_device *vfe_dev)
{
	int rc = 0;
	int i;
...
int legacy_m_msm_vfe47_enable_regulators(struct vfe_device *vfe_dev, int enable)
{
	return legacy_m_msm_camera_regulator_enable(vfe_dev->regulator_info,
					vfe_dev->vfe_num_regulators, enable);
}

int legacy_m_msm_vfe47_get_platform_data(struct vfe_device *vfe_dev)
{
	int rc = 0;

	vfe_dev->vfe_base = legacy_m_msm_camera_get_reg_base(vfe_dev->pdev, "vfe", 0);
	if (!vfe_dev->vfe_base)
		return -ENOMEM;
	vfe_dev->vfe_vbif_base = legacy_m_msm_camera_get_reg_base(vfe_dev->pdev,
					"vfe_vbif", 0);
	...
	vfe_dev->vfe_irq = legacy_m_msm_camera_get_irq(vfe_dev->pdev, "vfe");
	if (!vfe_dev->vfe_irq) {
		rc = -ENODEV;
		goto vfe_irq_fail;
	}

	vfe_dev->vfe_base_size = legacy_m_msm_camera_get_res_size(vfe_dev->pdev, "vfe");
	vfe_dev->vfe_vbif_base_size = legacy_m_msm_camera_get_res_size(vfe_dev->pdev,
						"vfe_vbif");
	...
	rc = legacy_m_msm_camera_register_irq(vfe_dev->pdev, vfe_dev->vfe_irq,
		legacy_m_msm_isp_process_irq,
	...
	rc = legacy_m_msm_isp_init_bandwidth_mgr(vfe_dev, ISP_VFE0 + vfe_dev->pdev->id);
	if (rc)
		goto init_bw_fail;
...
init_bw_fail:
	legacy_m_msm_camera_unregister_irq(vfe_dev->pdev, vfe_dev->vfe_irq, "vfe");
irq_register_fail:
...
vfe_irq_fail:
	legacy_m_msm_camera_put_reg_base(vfe_dev->pdev, vfe_dev->vfe_base,
					"vfe_vbif", 0);
vbif_base_fail:
	legacy_m_msm_camera_put_reg_base(vfe_dev->pdev, vfe_dev->vfe_base, "vfe", 0);
	return rc;
}

void legacy_m_msm_vfe47_get_error_mask(
...
void legacy_m_msm_vfe47_get_overflow_mask(uint32_t *overflow_mask)
{
	*overflow_mask = 0x09FFFE7E;
}

void legacy_m_msm_vfe47_get_rdi_wm_mask(struct vfe_device *vfe_dev,
	uint32_t *rdi_wm_mask)
{
	*rdi_wm_mask = vfe_dev->axi_data.rdi_wm_mask;
}

void legacy_m_msm_vfe47_get_irq_mask(struct vfe_device *vfe_dev,
	uint32_t *irq0_mask, uint32_t *irq1_mask)
{
...
void legacy_m_msm_vfe47_get_halt_restart_mask(uint32_t *irq0_mask,
	uint32_t *irq1_mask)
{
	*irq0_mask = BIT(31);
	*irq1_mask = BIT(8);
...
struct msm_vfe_hardware_info legacy_m_vfe47_hw_info = {
	.num_iommu_ctx = 1,
	...
	.vfe_ops = {
		...
		.core_ops = {
			...
			.start_fetch_eng_multi_pass =
				legacy_m_msm_vfe47_start_fetch_engine_multi_pass,
		},
		.stats_ops = {
		...
		.platform_ops = {
			...
			.get_regulators = legacy_m_msm_vfe47_get_regulators,
			.put_regulators = legacy_m_msm_vfe47_put_regulators,
			.enable_clks = legacy_m_msm_vfe47_enable_clks,
			.get_clks = legacy_m_msm_vfe47_get_clks,
			.put_clks = legacy_m_msm_vfe47_put_clks,
			.get_clk_rates = legacy_m_msm_vfe47_get_clk_rates,
			.get_max_clk_rate = legacy_m_msm_vfe47_get_max_clk_rate,
			.set_clk_rate = legacy_m_msm_vfe47_set_clk_rate,
			.init_bw_mgr = legacy_m_msm_vfe47_init_bandwidth_mgr,
			.deinit_bw_mgr = legacy_m_msm_vfe47_deinit_bandwidth_mgr,
			.update_bw = legacy_m_msm_vfe47_update_bandwidth,
		}
	},
