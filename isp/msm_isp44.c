		...
		enum msm_isp_irq_operation oper)
{
	...
	legacy_m_msm_camera_io_w_mb(irq0_mask, vfe_dev->vfe_base + 0x28);
	legacy_m_msm_camera_io_w_mb(irq1_mask, vfe_dev->vfe_base + 0x2C);
}
...
{
	uint32_t val;
	val = legacy_m_msm_camera_io_r(vfe_base + VFE44_WM_BASE(wm_idx));
	if (enable)
		val |= 0x1;
	else
		val &= ~0x1;
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	if (pix_cfg->input_mux == EXTERNAL_READ) {
		...
		msm_vfe44_config_irq(vfe_dev, (1 << 24), 0,
					MSM_ISP_IRQ_SET);
		legacy_m_msm_camera_io_w((fe_cfg->fetch_height - 1) & 0xFFF,
		...
		legacy_m_msm_camera_io_w(0xF6543210, vfe_dev->vfe_base + 0x248);
	...
	enum msm_isp_camif_update_state update_state)
{
	...
	if (update_state == ENABLE_CAMIF) {
		...
		legacy_m_msm_camera_io_w_mb(val & ~(1 << 8), vfe_dev->vfe_base + 0xC18);
	...
	uint8_t plane_idx)
{
	...
	xbar_reg_cfg =
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + VFE44_XBAR_BASE(wm));
...
static void msm_vfe44_stats_cfg_ub(struct vfe_device *vfe_dev)
{
	...
	for (i = 0; i < VFE44_NUM_STATS_TYPE; i++) {
		ub_offset -= ub_size[i];
		legacy_m_msm_camera_io_w(VFE44_STATS_BURST_LEN << 30 |
	...
	uint32_t stats_mask, uint8_t enable)
{
	...
	stats_cfg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x888);
	if (enable)
		stats_cfg |= stats_cfg_mask;
	else
		stats_cfg &= ~stats_cfg_mask;
	legacy_m_msm_camera_io_w(stats_cfg, vfe_dev->vfe_base + 0x888);
}
...
struct msm_vfe_hardware_info legacy_m_vfe44_hw_info = {
	.num_iommu_ctx = 1,
