...
spinlock_t land_msm_eventq_lock;

static struct pid *msm_pid;
spinlock_t land_msm_pid_lock;
...
EXPORT_SYMBOL(land_msm_cam_copy_v4l2_subdev_fops);
...
EXPORT_SYMBOL(land_msm_session_find);
...
EXPORT_SYMBOL(land_msm_create_stream);
...
EXPORT_SYMBOL(land_msm_delete_stream);
...
EXPORT_SYMBOL(land_msm_sd_register);

int land_msm_sd_unregister(struct msm_sd_subdev *msm_subdev)
{
	if (WARN_ON(!msm_subdev))
		return -EINVAL;
...
EXPORT_SYMBOL(land_msm_sd_unregister);
...
int land_msm_create_session(unsigned int session_id, struct video_device *vdev)
{
	struct msm_session *session = NULL;
...
EXPORT_SYMBOL(land_msm_create_session);
...
EXPORT_SYMBOL(land_msm_create_command_ack_q);
...
EXPORT_SYMBOL(land_msm_delete_command_ack_q);
...
EXPORT_SYMBOL(land_msm_destroy_session);
...
int land_msm_post_event(struct v4l2_event *event, int timeout)
{
	...
	spin_lock_irqsave(&land_msm_eventq_lock, flags);
	if (!msm_eventq) {
		spin_unlock_irqrestore(&land_msm_eventq_lock, flags);
...
EXPORT_SYMBOL(land_msm_post_event);
...
static int msm_close(struct file *filep)
{
	...
	spin_lock_irqsave(&land_msm_pid_lock, flags);
	put_pid(msm_pid);
	msm_pid = NULL;
	spin_unlock_irqrestore(&land_msm_pid_lock, flags);
...
EXPORT_SYMBOL(land_msm_get_stream);
...
EXPORT_SYMBOL(land_msm_get_stream_vb2q);
...
EXPORT_SYMBOL(land_msm_get_stream_from_vb2q);
	...
	unsigned int notification, void *arg)
{
	...
	case MSM_SD_NOTIFY_REQ_CB: {
		...
		rc = land_msm_vb2_request_cb(req_sd);
		if (rc < 0)
			return;
	}
		break;
...
static int msm_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&land_msm_eventq_lock);
	spin_lock_init(&land_msm_pid_lock);
