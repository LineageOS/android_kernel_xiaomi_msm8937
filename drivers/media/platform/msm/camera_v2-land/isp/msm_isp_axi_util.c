...
int land_msm_isp_validate_axi_request(struct msm_vfe_axi_shared_data *axi_data,
	struct msm_vfe_axi_stream_request_cmd *stream_cfg_cmd)
{
	int rc = -1, i;
	...
	default:
		land_msm_isp_print_fourcc_error(__func__,
...
void land_msm_isp_axi_reserve_wm(struct vfe_device *vfe_dev,
	struct msm_vfe_axi_shared_data *axi_data,
	struct msm_vfe_axi_stream *stream_info)
{
	int i, j;
...
void land_msm_isp_axi_free_wm(struct msm_vfe_axi_shared_data *axi_data,
	struct msm_vfe_axi_stream *stream_info)
{
	int i;
...
void land_msm_isp_axi_free_comp_mask(struct msm_vfe_axi_shared_data *axi_data,
	struct msm_vfe_axi_stream *stream_info)
{
...
/**
 * land_msm_isp_update_framedrop_reg() - Update frame period pattern on h/w
...
/**
 * land_msm_isp_reset_framedrop() - Compute the framedrop period pattern
	...
	struct msm_vfe_axi_stream *stream_info)
{
	...
	if (!stream_info->controllable_output) {
		stream_info->current_framedrop_period =
			land_msm_isp_get_framedrop_period(
	...
	if (!vfe_dev->reg_updated) {
		...
		for (i = 0; i < VFE_AXI_SRC_MAX; i++) {
			...
			if (temp_stream_info->undelivered_request_cnt) {
				if (land_msm_isp_drop_frame(vfe_dev, stream_info, ts,
					sof_info)) {
	...
	if ((dual_hw_type == DUAL_HW_MASTER_SLAVE) &&
		...
		if (frame_src == VFE_PIX_0) {
			...
			if (!src_info->frame_id &&
				...
				land_msm_isp_halt_send_error(vfe_dev,
	...
	enum msm_vfe_input_src frame_src, struct msm_isp_timestamp *ts)
{
	...
	land_msm_isp_send_event(vfe_dev, event_type | frame_src, &event_data);
}

/**
 * land_msm_isp_calculate_framedrop() - Setup frame period and pattern
	...
	struct msm_vfe_axi_stream_request_cmd *stream_cfg_cmd)
{
	...
	framedrop_period = land_msm_isp_get_framedrop_period(
...
void land_msm_isp_calculate_bandwidth(
	struct msm_vfe_axi_shared_data *axi_data,
	struct msm_vfe_axi_stream *stream_info)
{
	int bpp = 0;
	if (stream_info->stream_src < RDI_INTF_0) {
		...
		bpp = land_msm_isp_get_bit_per_pixel(stream_info->output_format);
		if (rdi < VFE_SRC_MAX)
			stream_info->bandwidth =
...
#ifdef CONFIG_MSM_AVTIMER
/**
 * land_msm_isp_set_avtimer_fptr() - Set avtimer function pointer
 ...
 *
 */
void land_msm_isp_set_avtimer_fptr(struct avtimer_fptr_t avtimer)
{
...
EXPORT_SYMBOL(land_msm_isp_set_avtimer_fptr);
...
void land_msm_isp_get_avtimer_ts(
...
static inline void land_msm_isp_get_avtimer_ts(
		struct msm_isp_timestamp *time_stamp)
{
...
int land_msm_isp_request_axi_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	rc = land_msm_isp_axi_create_stream(vfe_dev,
	...
	rc = land_msm_isp_validate_axi_request(
		&vfe_dev->axi_data, stream_cfg_cmd);
	if (rc) {
		pr_err("%s: Request validation failed\n", __func__);
		if (HANDLE_TO_IDX(stream_cfg_cmd->axi_stream_handle) <
			VFE_AXI_SRC_MAX)
			land_msm_isp_axi_destroy_stream(&vfe_dev->axi_data,
			      HANDLE_TO_IDX(stream_cfg_cmd->axi_stream_handle));
		return rc;
	}
	...
	land_msm_isp_axi_reserve_wm(vfe_dev, &vfe_dev->axi_data, stream_info);
	...
	rc = land_msm_isp_calculate_framedrop(&vfe_dev->axi_data, stream_cfg_cmd);
	if (rc)
		goto done;
	if (stream_cfg_cmd->vt_enable && !vfe_dev->vt_enable) {
		vfe_dev->vt_enable = stream_cfg_cmd->vt_enable;
		land_msm_isp_start_avtimer();
	}
	if (stream_info->num_planes > 1) {
		land_msm_isp_axi_reserve_comp_mask(
	...
	if (rc) {
		land_msm_isp_axi_free_wm(&vfe_dev->axi_data, stream_info);
		land_msm_isp_axi_destroy_stream(&vfe_dev->axi_data,
			HANDLE_TO_IDX(stream_cfg_cmd->axi_stream_handle));
	}
	return rc;
...
int land_msm_isp_release_axi_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	if (stream_info->state == AVAILABLE) {
		...
		land_msm_isp_cfg_axi_stream(vfe_dev, (void *) &stream_cfg);
	}
	...
	if (stream_info->num_planes > 1) {
		...
		land_msm_isp_axi_free_comp_mask(&vfe_dev->axi_data, stream_info);
	} else {
		vfe_dev->hw_info->vfe_ops.axi_ops.
	...
	land_msm_isp_axi_free_wm(axi_data, stream_info);
		...
		struct msm_vfe_axi_stream *stream_info)
{
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < stream_info->num_planes; j++) {
			word_per_line = land_msm_isp_cal_word_per_line(
...
void land_msm_isp_halt_send_error(struct vfe_device *vfe_dev, uint32_t event)
{
	...
	land_msm_isp_send_event(vfe_dev, event, &error_event);
}
	...
	int scratch)
{
	...
	if ((stream_info->stream_src < RDI_INTF_0) &&
		...
		land_msm_isp_halt_send_error(vfe_dev, ISP_EVENT_BUF_FATAL_ERROR);
		return 0;
	}
	...
	uint32_t frame_id)
{
	...
	land_msm_isp_send_event(vfe_dev, ISP_EVENT_ERROR,
		&error_event);
	...
	struct timeval *time_stamp, uint32_t frame_id)
{
	...
	if (stream_info->buf_divert &&
		...
		if ((bufq != NULL) && bufq->buf_type == ISP_SHARE_BUF)
			land_msm_isp_send_event(vfe_dev->common_data->
		...
		ISP_DBG("%s: vfe_id %d send buf done buf-id %d bufq %x\n",
			...
			buf->bufq_handle);
		land_msm_isp_send_event(vfe_dev, ISP_EVENT_BUF_DONE,
			&buf_event);
		buf->buf_debug.put_state[
			buf->buf_debug.put_state_last] =
	...
	struct msm_isp_sof_info *sof_info)
{
	...
	if (stream_info->activated_framedrop_period ==
		MSM_VFE_STREAM_STOP_PERIOD) {
		/* no buf done come */
		land_msm_isp_process_axi_irq_stream(vfe_dev, stream_info,
			pingpong_status, ts);
	}
	return 0;
}
...
static int msm_isp_update_stream_bandwidth(struct vfe_device *vfe_dev)
{
	...
	rc = land_msm_isp_update_bandwidth(ISP_VFE0 + vfe_dev->pdev->id,
	...
	struct msm_vfe_axi_reset_cmd *reset_cmd)
{
	...
	land_msm_isp_get_timestamp(&timestamp);
	...
	for (i = 0, j = 0; j < axi_data->num_active_stream &&
		...
		for (bufq_id = 0; bufq_id < VFE_BUF_QUEUE_MAX; bufq_id++) {
			...
			land_msm_isp_reset_burst_count_and_frame_drop(vfe_dev,
				stream_info);
		}
	}
			...
			enum msm_isp_camif_update_state camif_update)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		land_msm_isp_calculate_bandwidth(axi_data, stream_info);
		msm_isp_get_stream_wm_mask(stream_info, &wm_reload_mask);
		spin_lock_irqsave(&stream_info->lock, flags);
...
int land_msm_isp_cfg_axi_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	rc = land_msm_isp_axi_check_stream_state(vfe_dev, stream_cfg_cmd);
	if (rc < 0) {
	...
	uint32_t frame_id, enum msm_vfe_input_src frame_src)
{
	...
	error_event.u.error_info.stream_id_mask =
		1 << (bufq_handle & 0xFF);
	land_msm_isp_send_event(vfe_dev, ISP_EVENT_ERROR, &error_event);
		...
		struct msm_isp_timestamp *ts)
{
	...
	if (vfe_dev->vt_enable) {
		land_msm_isp_get_avtimer_ts(ts);
