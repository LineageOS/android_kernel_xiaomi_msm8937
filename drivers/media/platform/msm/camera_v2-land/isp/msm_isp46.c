...
static int msm_vfe46_init_hardware(struct vfe_device *vfe_dev)
{
	...
	rc = land_msm_cam_clk_enable(&vfe_dev->pdev->dev, msm_vfe46_clk_info,
...
vfe_remap_failed:
	land_msm_cam_clk_enable(&vfe_dev->pdev->dev, msm_vfe46_clk_info,
...
static void msm_vfe46_init_hardware_reg(struct vfe_device *vfe_dev)
{
	...
	/* BUS_CFG */
	land_msm_camera_io_w(0x00000001, vfe_dev->vfe_base + 0x84);
	/* IRQ_MASK/CLEAR */
	land_msm_camera_io_w(0xE00000F1, vfe_dev->vfe_base + 0x5C);
	land_msm_camera_io_w_mb(0xE1FFFFFF, vfe_dev->vfe_base + 0x60);
	...
	uint32_t irq_status0, uint32_t irq_status1)
{
	if (irq_status1 & (1 << 8)) {
		...
		land_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x374);
	}
}
...
static void msm_vfe46_process_error_status(struct vfe_device *vfe_dev)
{
	...
	if (error_status1 & (1 << 0)) {
		...
		land_msm_camera_io_dump(vfe_dev->vfe_base + 0x3A8, 0x30, 1);
	}
	...
	uint32_t *irq_status0, uint32_t *irq_status1)
{
	...
	*irq_status1 &= irq_mask1;
	...
	if (*irq_status1 & (1 << 0)) {
		...
		land_msm_camera_io_r(vfe_dev->vfe_base + 0x3D0);
	...
	uint32_t first_start, uint32_t blocking_call)
{
	...
	if (first_start) {
		land_msm_camera_io_w_mb(0x1FF, vfe_dev->vfe_base + 0x18);
	} else {
		land_msm_camera_io_w_mb(0x1EF, vfe_dev->vfe_base + 0x18);
	...
	uint8_t wm_idx, uint8_t enable)
{
	...
	val = land_msm_camera_io_r(vfe_base + VFE46_WM_BASE(wm_idx));
	if (enable)
		val |= 0x1;
	else
		val &= ~0x1;
	...
	void *arg)
{
	...
	land_msm_camera_io_w(mapped_info.paddr, vfe_dev->vfe_base + 0x268);
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	if (pix_cfg->input_mux == EXTERNAL_READ) {
		...
		land_msm_camera_io_w(temp & 0x3FFF, vfe_dev->vfe_base + 0x278);
		...
		land_msm_camera_io_w(temp, vfe_dev->vfe_base + 0x284);

		/* need to use formulae to calculate MAIN_UNPACK_PATTERN*/
		land_msm_camera_io_w(0xF6543210, vfe_dev->vfe_base + 0x288);
		land_msm_camera_io_w(0xF, vfe_dev->vfe_base + 0x2A4);
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	if (bus_sub_en) {
		val = land_msm_camera_io_r(vfe_dev->vfe_base + 0x3AC);
		val &= 0xFFFFFFDF;
		val = val | bus_sub_en << 5;
		land_msm_camera_io_w(val, vfe_dev->vfe_base + 0x3AC);
	...
	if (subsample_period && subsample_pattern) {
		...
		land_msm_camera_io_w(val, vfe_dev->vfe_base + 0x3C8);
	} else {
		land_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x3C8);
	}

	val = land_msm_camera_io_r(vfe_dev->vfe_base + 0x39C);
	val |= camif_cfg->camif_input;
	land_msm_camera_io_w(val, vfe_dev->vfe_base + 0x39C);
}
	...
	enum msm_isp_camif_update_state update_state)
{
	...
	if (update_state == ENABLE_CAMIF) {
		...
		land_msm_camera_io_w(val, vfe_dev->vfe_base + 0x3AC);
		land_msm_camera_io_w_mb(0x4, vfe_dev->vfe_base + 0x3A8);
		land_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x3A8);
		/* configure EPOCH0 for 20 lines */
		land_msm_camera_io_w_mb(0x140000, vfe_dev->vfe_base + 0x3CC);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 1;
		/* testgen GO*/
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			land_msm_camera_io_w(1, vfe_dev->vfe_base + 0xAF4);
	} else if (update_state == DISABLE_CAMIF) {
		land_msm_camera_io_w_mb(0x0, vfe_dev->vfe_base + 0x3A8);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 0;
		/* testgen OFF*/
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			land_msm_camera_io_w(1 << 1, vfe_dev->vfe_base + 0xAF4);
	} else if (update_state == DISABLE_CAMIF_IMMEDIATELY) {
		land_msm_camera_io_w_mb(0x6, vfe_dev->vfe_base + 0x3A8);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 0;
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			land_msm_camera_io_w(1 << 1, vfe_dev->vfe_base + 0xAF4);
	}
	...
	uint8_t plane_idx)
{
	...
	val = land_msm_camera_io_r(vfe_dev->vfe_base + wm_base + 0xC);
	val &= ~0x2;
	...
	uint8_t plane_idx)
{
	...
	xbar_reg_cfg =
		land_msm_camera_io_r(vfe_dev->vfe_base + VFE46_XBAR_BASE(wm));
	...
	uint32_t blocking)
{
	...
	if (blocking) {
		...
		/* Halt AXI Bus Bridge */
		land_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x374);
		rc = wait_for_completion_timeout(
	...
	uint32_t blocking, uint32_t enable_camif)
{
	...
	land_msm_camera_io_w(0xFFFFFEFF, vfe_dev->vfe_base + 0x68);
	land_msm_camera_io_w(0x1, vfe_dev->vfe_base + 0x58);
	land_msm_camera_io_w_mb(0x20000, vfe_dev->vfe_base + 0x3CC);
...
static uint32_t msm_vfe46_get_pingpong_status(
	struct vfe_device *vfe_dev)
{
	return land_msm_camera_io_r(vfe_dev->vfe_base + 0x2A8);
}
...
static void msm_vfe46_stats_cfg_ub(struct vfe_device *vfe_dev)
{
	...
	for (i = 0; i < VFE46_NUM_STATS_TYPE; i++) {
		ub_offset -= ub_size[i];
		land_msm_camera_io_w(VFE46_STATS_BURST_LEN << 30 |
...
struct msm_vfe_hardware_info land_vfe46_hw_info = {
	.num_iommu_ctx = 1,
