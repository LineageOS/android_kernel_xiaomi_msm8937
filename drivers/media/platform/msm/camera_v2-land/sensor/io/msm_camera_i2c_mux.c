...
{
	uint32_t val;
	val = land_msm_camera_io_r(mux_device->ctl_base);
	if (*mode == MODE_DUAL) {
		land_msm_camera_io_w(val | 0x3, mux_device->ctl_base);
	} else if (*mode == MODE_L) {
		land_msm_camera_io_w(((val | 0x2) & ~(0x1)), mux_device->ctl_base);
		val = land_msm_camera_io_r(mux_device->ctl_base);
		CDBG("the camio mode config left value is %d\n", val);
	} else {
		land_msm_camera_io_w(((val | 0x1) & ~(0x2)), mux_device->ctl_base);
		val = land_msm_camera_io_r(mux_device->ctl_base);
...
static int msm_i2c_mux_init(struct i2c_mux_device *mux_device)
{
	...
	if (mux_device->use_count == 0) {
		...
		val = land_msm_camera_io_r(mux_device->rw_base);
		land_msm_camera_io_w((val | 0x200), mux_device->rw_base);
	}
