	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	land_msm_camera_io_w(csiphy_dev->ctrl_reg->csiphy_3ph_reg.
	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	land_msm_camera_io_w(lane_enable,
		csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
	...
	while (lane_mask & 0x7) {
		...
		land_msm_camera_io_w(((csiphy_params->settle_cnt >> 8) & 0xff),
			csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
			mipi_csiphy_3ph_lnn_ctrl2.addr + 0x200*i);
		land_msm_camera_io_w((csiphy_params->settle_cnt & 0xff),
			csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
	...
	if (csiphy_params->combo_mode == 1) {
		land_msm_camera_io_w(0x2,
			csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
			mipi_csiphy_3ph_cmn_ctrl7.addr);
	} else {
		land_msm_camera_io_w(0x6,
			csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
			mipi_csiphy_3ph_cmn_ctrl7.addr);
	}
	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	for (i = 0, mask = 0x1; i < MAX_LANES; i++) {
		...
		land_msm_camera_io_w(val, csiphybase +
	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	if (csiphy_dev->hw_version >= CSIPHY_VERSION_V30) {
		val = land_msm_camera_io_r(csiphy_dev->clk_mux_base);
		...
		land_msm_camera_io_w(val, csiphy_dev->clk_mux_base);
	...
	if (csiphy_dev->csiphy_3phase == CSI_3PHASE_HW) {
		if (csiphy_params->csi_3phase == 1) {
			land_msm_cam_clk_enable(&csiphy_dev->pdev->dev,
	...
	land_msm_camera_io_w(0x1, csiphybase + csiphy_dev->ctrl_reg->
	...
	if (csiphy_dev->hw_version < CSIPHY_VERSION_V30) {
		val = 0x3;
		land_msm_camera_io_w((lane_mask << 2) | val,
				csiphybase +
				csiphy_dev->ctrl_reg->
				csiphy_reg.mipi_csiphy_glbl_pwr_cfg_addr);
		land_msm_camera_io_w(0x10, csiphybase +
			csiphy_dev->ctrl_reg->csiphy_reg.
			mipi_csiphy_lnck_cfg2_addr);
		land_msm_camera_io_w(csiphy_params->settle_cnt,
			csiphybase +
			csiphy_dev->ctrl_reg->csiphy_reg.
			mipi_csiphy_lnck_cfg3_addr);
		land_msm_camera_io_w(0x24,
			csiphybase + csiphy_dev->ctrl_reg->
		...
		val = 0x1;
		land_msm_camera_io_w((lane_mask << 1) | val,
				csiphybase +
				csiphy_dev->ctrl_reg->
				csiphy_reg.mipi_csiphy_glbl_pwr_cfg_addr);
		land_msm_camera_io_w(csiphy_params->combo_mode <<
			csiphy_dev->ctrl_reg->csiphy_reg.
	...
	while (lane_mask & 0x1f) {
		...
		if (csiphy_dev->is_3_1_20nm_hw == 1) {
			...
			land_msm_camera_io_w(lane_val, csiphybase +
				csiphy_dev->ctrl_reg->csiphy_reg.
				mipi_csiphy_lnn_misc1_addr + 0x40*j);
			land_msm_camera_io_w(0x17, csiphybase +
				csiphy_dev->ctrl_reg->csiphy_reg.
...
static irqreturn_t msm_csiphy_irq(int irq_num, void *data)
{
	...
	for (i = 0; i < csiphy_dev->num_irq_registers; i++) {
		irq = land_msm_camera_io_r(
			csiphy_dev->base +
			csiphy_dev->ctrl_reg->csiphy_reg.
			mipi_csiphy_interrupt_status0_addr + 0x4*i);
		land_msm_camera_io_w(irq,
			csiphy_dev->base +
			csiphy_dev->ctrl_reg->csiphy_reg.
		...
		land_msm_camera_io_w(0x0,
			csiphy_dev->base +
			csiphy_dev->ctrl_reg->csiphy_reg.
			mipi_csiphy_interrupt_clear0_addr + 0x4*i);
	}
	land_msm_camera_io_w(0x1, csiphy_dev->base +
		csiphy_dev->ctrl_reg->
		csiphy_reg.mipi_csiphy_glbl_irq_cmd_addr);
	land_msm_camera_io_w(0x0, csiphy_dev->base +
		csiphy_dev->ctrl_reg->
		csiphy_reg.mipi_csiphy_glbl_irq_cmd_addr);
	return IRQ_HANDLED;
}
...
static int msm_csiphy_init(struct csiphy_device *csiphy_dev)
{
	...
	rc = land_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_CSIPHY,
			CAM_AHB_SVS_VOTE);
	if (rc < 0) {
		csiphy_dev->ref_count--;
	...
	if (csiphy_dev->hw_dts_version < CSIPHY_VERSION_V30) {
		rc = land_msm_cam_clk_enable(&csiphy_dev->pdev->dev,
	...
	if (csiphy_dev->hw_dts_version == CSIPHY_VERSION_V30)
		csiphy_dev->hw_version =
			land_msm_camera_io_r(csiphy_dev->base +
				 csiphy_dev->ctrl_reg->
...
ioremap_fail:
	csiphy_dev->ref_count--;
	if (land_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_CSIPHY,
...
static int csiphy_probe(struct platform_device *pdev)
{
	...
	land_msm_sd_register(&new_csiphy_dev->msm_sd);
	...
	if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		...
		"qcom,csiphy-v3.2")) {
		new_csiphy_dev->ctrl_reg->csiphy_reg = land_csiphy_v3_2;
		new_csiphy_dev->hw_dts_version = CSIPHY_VERSION_V32;
	} else if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v3.4.2")) {
		new_csiphy_dev->ctrl_reg->csiphy_3ph_reg = land_csiphy_v3_4_2_3ph;
		new_csiphy_dev->ctrl_reg->csiphy_reg = land_csiphy_v3_4_2;
		new_csiphy_dev->hw_dts_version = CSIPHY_VERSION_V342;
		new_csiphy_dev->csiphy_3phase = CSI_3PHASE_HW;
	} else if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v3.5")) {
		new_csiphy_dev->ctrl_reg->csiphy_3ph_reg = land_csiphy_v3_5_3ph;
		new_csiphy_dev->ctrl_reg->csiphy_reg = land_csiphy_v3_5;
	...
	land_msm_cam_copy_v4l2_subdev_fops(&msm_csiphy_v4l2_subdev_fops);
#ifdef CONFIG_COMPAT
