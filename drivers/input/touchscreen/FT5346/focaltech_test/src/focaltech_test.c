...
* functions body
*******************************************************************************/
extern struct i2c_client *ft5346_i2c_client;
...
static int fts_test_i2c_read(unsigned char *writebuf, int writelen, unsigned char *readbuf, int readlen)
{
	...
	if (writelen > 0) {
		 ...
		 ret = i2c_transfer(ft5346_i2c_client->adapter, msgs, 2);
		 if (ret < 0)
			dev_err(&ft5346_i2c_client->dev, "%s: i2c read error.\n", __func__);
	} else {
		 struct i2c_msg msgs[] = {
			{
				.addr = ft5346_i2c_client->addr,
		 ...
		 ret = i2c_transfer(ft5346_i2c_client->adapter, msgs, 1);
		 if (ret < 0)
			dev_err(&ft5346_i2c_client->dev, "%s:i2c read error.\n", __func__);
	}
...
static int fts_test_i2c_write(unsigned char *writebuf, int writelen)
{
	...
	ret = i2c_transfer(ft5346_i2c_client->adapter, msgs, 1);
	if (ret < 0)
		 dev_err(&ft5346_i2c_client->dev, "%s: i2c write error.\n", __func__);
