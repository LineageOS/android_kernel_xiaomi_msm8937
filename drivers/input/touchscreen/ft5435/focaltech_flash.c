...
struct ft_chip_t ft5435_chip_types;
...
struct fts_upgrade_fun  ft5435_fts_updatefun_curr;
struct workqueue_struct *ft5435_touch_wq;
struct work_struct ft5435_fw_update_work;
u8 *ft5435_g_fw_file;
int ft5435_g_fw_len;
...
int ft5435_fts_ctpm_i2c_hid2std(struct i2c_client *client)
{
	...
	buf[1] = 0xaa;
	buf[2] = 0x09;
	bRet =ft5435_fts_i2c_write(client, buf, 3);
	msleep(10);
	buf[0] = buf[1] = buf[2] = 0;
	ft5435_fts_i2c_read(client, buf, 0, buf, 3);
...
void fts_get_ft5435_chip_types(void)
{
	...
	ft5435_chip_types = ctype[ic_type];

	FTS_INFO("CHIP TYPE ID = 0x%02x%02x", ft5435_chip_types.chip_idh, ft5435_chip_types.chip_idl);
}
...
void ft5435_fts_ctpm_get_upgrade_array(void)
{
	...
	fts_get_ft5435_chip_types();

	ft5435_fts_ctpm_i2c_hid2std(ft5435_fts_i2c_client);


	memcpy(&ft5435_fts_updatefun_curr, &fts_updatefun, sizeof(struct fts_upgrade_fun));
...
void fts_LockDownInfo_get(struct i2c_client *client, char *pProjectCode)
{
	...
	ft5435_ft5435_fts_i2c_read_reg(client, FTS_REG_VENDOR_ID, &uc_tp_vendor_id);
	FTS_DEBUG("ft5435_ctpm_LockDownInfo_get_from_boot, uc_tp_vendor_id=%x\n", uc_tp_vendor_id);

	i_ret = ft5435_fts_ctpm_i2c_hid2std(client);
	if (i_ret == 0) {
		FTS_ERROR("HidI2c change to StdI2c fail ! \n");
	}
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {

		ft5435_ft5435_fts_i2c_write_reg(client, 0xfc, FTS_UPGRADE_AA);
		msleep(30);
		ft5435_ft5435_fts_i2c_write_reg(client, 0xfc, FTS_UPGRADE_55);
		msleep(200);
	...
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) {
		...
		i_ret = ft5435_fts_i2c_write(client, auc_i2c_write_buf, 4);
		if (i_ret < 0) {
		...
		i_ret = ft5435_fts_i2c_read(client, auc_i2c_write_buf, 0, reg_val, 8);
		if (i_ret < 0) {
	...
	i_ret = ft5435_fts_ctpm_i2c_hid2std(client);   /* Android to Std i2c. */
	if (i_ret == 0) {
		FTS_ERROR("HidI2c change to StdI2c fail ! \n");
	}
	msleep(10);
...
void ft5435_fts_ctpm_rom_or_pram_reset(struct i2c_client *client)
{
	...
	ft5435_fts_i2c_write(client, &rst_cmd, 1);
...
int ft5435_fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	ft5435_ft5435_fts_i2c_write_reg(client, 0, FTS_REG_WORKMODE_FACTORY_VALUE);

	msleep(100);

	ft5435_ft5435_fts_i2c_write_reg(client, 2, 0x4);
	msleep(300);
	if ((ft5435_chip_types.chip_idh==0x11) ||(ft5435_chip_types.chip_idh==0x12) ||(ft5435_chip_types.chip_idh==0x13) ||(ft5435_chip_types.chip_idh==0x14)) {
		for (i=0; i<100; i++) {
			ft5435_ft5435_fts_i2c_read_reg(client, 0x02, &uc_temp);
			if (0x02 == uc_temp ||
				0xFF == uc_temp) {
				break;
			}
		...
		for (i=0; i<100; i++) {
			ft5435_ft5435_fts_i2c_read_reg(client, 0, &uc_temp);
	...
	ft5435_ft5435_fts_i2c_write_reg(client, 0, 0x40);
	msleep(200);
	ft5435_ft5435_fts_i2c_write_reg(client, 2, 0x5);
	msleep(300);
	ft5435_ft5435_fts_i2c_write_reg(client, 0, FTS_REG_WORKMODE_WORK_VALUE);
	msleep(300);
#endif
...
u32 ft5435_fts_getsize(u8 fw_type)
{
	...
	if (fw_type == FW_SIZE)
		...
		fw_len = sizeof(ft5435_aucFW_PRAM_BOOT);
#endif
...
enum FW_STATUS ft5435_fts_ctpm_get_pram_or_rom_id(struct i2c_client *client)
{
	...
	ft5435_fts_i2c_write(client, buf, 2);

	msleep(20);

	buf[0] = 0x90;
	buf[1] = buf[2] = buf[3] =0x00;
	ft5435_fts_i2c_read(client, buf, 4, reg_val, 2);
	...
	} else if (reg_val[0] == ft5435_chip_types.pramboot_idh && reg_val[1] == ft5435_chip_types.pramboot_idl) {
		inRomBoot = FTS_RUN_IN_PRAM;
	} else if (reg_val[0] == ft5435_chip_types.rom_idh && reg_val[1] == ft5435_chip_types.rom_idl) {
		inRomBoot = FTS_RUN_IN_ROM;
	} else if (reg_val[0] == ft5435_chip_types.bootloader_idh && reg_val[1] == ft5435_chip_types.bootloader_idl) {
...
int fts_ctpm_get_i_file(struct i2c_client *client, int fw_valid)
{
	...
	if (ft5435_fts_updatefun_curr.get_i_file)
		ret = ft5435_fts_updatefun_curr.get_i_file(client, fw_valid);
	else
		ret = -EIO;
...
int ft5435_fts_ctpm_get_app_ver(void)
{
	...
	if (ft5435_fts_updatefun_curr.get_app_i_file_ver)
		i_ret = ft5435_fts_updatefun_curr.get_app_i_file_ver();
...
int ft5435_fts_ctpm_fw_upgrade(struct i2c_client *client)
{
	...
	if (ft5435_fts_updatefun_curr.upgrade_with_app_i_file)
		i_ret = ft5435_fts_updatefun_curr.upgrade_with_app_i_file(client);
...
int ft5435_fts_ctpm_lcd_cfg_upgrade(struct i2c_client *client)
{
	...
	if (ft5435_fts_updatefun_curr.upgrade_with_lcd_cfg_i_file)
		i_ret = ft5435_fts_updatefun_curr.upgrade_with_lcd_cfg_i_file(client);
...
static int fts_ctpm_check_fw_status(struct i2c_client *client)
{
	...
	for (i = 0; i < 5; i++) {
		ret = ft5435_ft5435_fts_i2c_read_reg(client, FTS_REG_CHIP_ID, &chip_id1);
		if (ret < 0) {
			i2c_noack_retry++;
			continue;
		}
		ret = ft5435_ft5435_fts_i2c_read_reg(client, FTS_REG_CHIP_ID2, &chip_id2);
		if (ret < 0) {
			i2c_noack_retry++;
			continue;
		}

		if (chip_id1 == ft5435_chip_types.chip_idh
#if FTS_CHIP_IDC
			&& chip_id2 == ft5435_chip_types.chip_idl
#endif
		   ) {
	...
	FTS_DEBUG("[UPGRADE]: chip_id = %02x%02x, ft5435_chip_types.chip_idh = %02x%02x",
			 chip_id1, chip_id2, ft5435_chip_types.chip_idh, ft5435_chip_types.chip_idl);
	if (i2c_noack_retry >= 5)
		return -EIO;
	if (i >= 5) {
		fw_status = ft5435_fts_ctpm_get_pram_or_rom_id(client);
	}
...
static int fts_ctpm_check_fw_ver(struct i2c_client *client)
{
	...
	uc_host_fm_ver = ft5435_fts_ctpm_get_app_ver();
...
int ft5435_fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if (bUpgradeFlag <= 0) {
		...
		sprintf(tp_info_buff, "[FW]0x%02x, [IC]FT5435", uc_tp_fm_ver);
				...
				ft5435_fts_ctpm_auto_clb(client);
				break;
			} else {
...
static void fts_ctpm_update_work_func(struct work_struct *work)
{
	...
	i_ret = ft5435_fts_ctpm_auto_upgrade(ft5435_fts_i2c_client);
	if (i_ret < 0)
	...
	i_ret = ft5435_fts_ctpm_lcd_cfg_upgrade(ft5435_fts_i2c_client);
	if (i_ret < 0)
...
void fts_ctpm_upgrade_init(void)
{
	...
	ft5435_touch_wq = create_singlethread_workqueue("ft5435_touch_wq");
	if (ft5435_touch_wq) {
		INIT_WORK(&ft5435_fw_update_work, fts_ctpm_update_work_func);
		queue_work(ft5435_touch_wq, &ft5435_fw_update_work);
	} else {
...
{
	FTS_FUNC_ENTER();
	destroy_workqueue(ft5435_touch_wq);
	FTS_FUNC_EXIT();
}
int ft5435_fts_ctpm_fw_upgrade_ReadBootloadorID(struct i2c_client *client)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		reg_val[0] = reg_val[1] = 0x00;
		ft5435_fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
		if (reg_val[0] == ft5435_chip_types.chip_idh) {
			FTS_DEBUG("[FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x!!",
					 reg_val[0], reg_val[1]);
			return 0;
		} else {
