...
static irqreturn_t msm_vpe_irq(int irq_num, void *data)
{
	...
	irq_status = legacy_m_msm_camera_io_r_mb(vpe_dev->base +
	...
	legacy_m_msm_camera_io_w_mb(irq_status, vpe_dev->base + VPE_INTR_CLEAR_OFFSET);
	legacy_m_msm_camera_io_w(0, vpe_dev->base + VPE_INTR_ENABLE_OFFSET);
...
static int vpe_init_hardware(struct vpe_device *vpe_dev)
{
	...
	rc = legacy_m_msm_cam_clk_enable(&vpe_dev->pdev->dev, vpe_clk_info,
	...
	vpe_dev->buf_mgr_subdev = legacy_m_msm_buf_mngr_get_subdev();
...
static int vpe_release_hardware(struct vpe_device *vpe_dev)
{
	...
	iounmap(vpe_dev->base);
	legacy_m_msm_cam_clk_enable(&vpe_dev->pdev->dev, vpe_clk_info,
			...
			struct msm_vpe_frame_strip_info strip_info)
{
	...
	*/
	 ...
	 * improved.
	 */
	temp = legacy_m_msm_camera_io_r(vpe_dev->base + VPE_OP_MODE_OFFSET) | 0x3;
	legacy_m_msm_camera_io_w(temp, vpe_dev->base + VPE_OP_MODE_OFFSET);
	...
	legacy_m_msm_camera_io_w(src_roi, vpe_dev->base + VPE_SRC_SIZE_OFFSET);
	...
	legacy_m_msm_camera_io_w(src_xy, vpe_dev->base +
			VPE_SRC_XY_OFFSET);
	...
	legacy_m_msm_camera_io_w(strip_info.phase_step_x, vpe_dev->base +
			VPE_SCALE_PHASEX_STEP_OFFSET);
	legacy_m_msm_camera_io_w(strip_info.phase_step_y, vpe_dev->base +
			VPE_SCALE_PHASEY_STEP_OFFSET);

	legacy_m_msm_camera_io_w(strip_info.phase_init_x, vpe_dev->base +
			VPE_SCALE_PHASEX_INIT_OFFSET);
	legacy_m_msm_camera_io_w(strip_info.phase_init_y, vpe_dev->base +
	...
	unsigned long outP1)
{
	...
	legacy_m_msm_camera_io_w(srcP0, vpe_dev->base + VPE_SRCP0_ADDR_OFFSET);
	legacy_m_msm_camera_io_w(srcP1, vpe_dev->base + VPE_SRCP1_ADDR_OFFSET);
	legacy_m_msm_camera_io_w(outP0, vpe_dev->base + VPE_OUTP0_ADDR_OFFSET);
	legacy_m_msm_camera_io_w(outP1, vpe_dev->base + VPE_OUTP1_ADDR_OFFSET);
}

static int vpe_start(struct vpe_device *vpe_dev)
{
	/*  enable the frame irq, bit 0 = Display list 0 ROI done */
	legacy_m_msm_camera_io_w_mb(1, vpe_dev->base + VPE_INTR_ENABLE_OFFSET);
	legacy_m_msm_camera_io_dump(vpe_dev->base, 0x120, CONFIG_MSM_VPE_DBG);
	legacy_m_msm_camera_io_dump(vpe_dev->base + 0x00400, 0x18, CONFIG_MSM_VPE_DBG);
	legacy_m_msm_camera_io_dump(vpe_dev->base + 0x10000, 0x250, CONFIG_MSM_VPE_DBG);
	legacy_m_msm_camera_io_dump(vpe_dev->base + 0x30000, 0x20, CONFIG_MSM_VPE_DBG);
	legacy_m_msm_camera_io_dump(vpe_dev->base + 0x50000, 0x30, CONFIG_MSM_VPE_DBG);
	legacy_m_msm_camera_io_dump(vpe_dev->base + 0x50400, 0x10, CONFIG_MSM_VPE_DBG);

	/*
	 * This triggers the operation. When the VPE is done,
	 * msm_vpe_irq will fire.
	 */
	legacy_m_msm_camera_io_w_mb(1, vpe_dev->base + VPE_DL0_START_OFFSET);
	return 0;
}

static void vpe_config_axi_default(struct vpe_device *vpe_dev)
{
	legacy_m_msm_camera_io_w(0x25, vpe_dev->base + VPE_AXI_ARB_2_OFFSET);
}
...
static int vpe_reset(struct vpe_device *vpe_dev)
{
	...
	vpe_version = legacy_m_msm_camera_io_r(
			vpe_dev->base + VPE_HW_VERSION_OFFSET);
	VPE_DBG("vpe_version = 0x%x\n", vpe_version);
	/* disable all interrupts.*/
	legacy_m_msm_camera_io_w(0, vpe_dev->base + VPE_INTR_ENABLE_OFFSET);
	/* clear all pending interrupts*/
	legacy_m_msm_camera_io_w(0x1fffff, vpe_dev->base + VPE_INTR_CLEAR_OFFSET);
	/* write sw_reset to reset the core. */
	legacy_m_msm_camera_io_w(0x10, vpe_dev->base + VPE_SW_RESET_OFFSET);
	/* then poll the reset bit, it should be self-cleared. */
	while (1) {
		rc = legacy_m_msm_camera_io_r(
	 ...
	 * values.
	 */
	legacy_m_msm_camera_io_w(VPE_AXI_RD_ARB_CONFIG_VALUE,
			vpe_dev->base + VPE_AXI_RD_ARB_CONFIG_OFFSET);

	legacy_m_msm_camera_io_w(VPE_CGC_ENABLE_VALUE,
			vpe_dev->base + VPE_CGC_EN_OFFSET);
	legacy_m_msm_camera_io_w(1, vpe_dev->base + VPE_CMD_MODE_OFFSET);
	legacy_m_msm_camera_io_w(VPE_DEFAULT_OP_MODE_VALUE,
			vpe_dev->base + VPE_OP_MODE_OFFSET);
	legacy_m_msm_camera_io_w(VPE_DEFAULT_SCALE_CONFIG,
...
{
	legacy_m_msm_camera_io_w(*p, vpe_dev->base + VPE_SRC_FORMAT_OFFSET);
	legacy_m_msm_camera_io_w(*(++p),
		vpe_dev->base + VPE_SRC_UNPACK_PATTERN1_OFFSET);
	legacy_m_msm_camera_io_w(*(++p), vpe_dev->base + VPE_SRC_IMAGE_SIZE_OFFSET);
	legacy_m_msm_camera_io_w(*(++p), vpe_dev->base + VPE_SRC_YSTRIDE1_OFFSET);
	legacy_m_msm_camera_io_w(*(++p), vpe_dev->base + VPE_SRC_SIZE_OFFSET);
	legacy_m_msm_camera_io_w(*(++p), vpe_dev->base + VPE_SRC_XY_OFFSET);
}

static void vpe_output_plane_config(struct vpe_device *vpe_dev, uint32_t *p)
{
	legacy_m_msm_camera_io_w(*p, vpe_dev->base + VPE_OUT_FORMAT_OFFSET);
	legacy_m_msm_camera_io_w(*(++p),
		vpe_dev->base + VPE_OUT_PACK_PATTERN1_OFFSET);
	legacy_m_msm_camera_io_w(*(++p), vpe_dev->base + VPE_OUT_YSTRIDE1_OFFSET);
	legacy_m_msm_camera_io_w(*(++p), vpe_dev->base + VPE_OUT_SIZE_OFFSET);
	legacy_m_msm_camera_io_w(*(++p), vpe_dev->base + VPE_OUT_XY_OFFSET);
}

static void vpe_operation_config(struct vpe_device *vpe_dev, uint32_t *p)
{
	legacy_m_msm_camera_io_w(*p, vpe_dev->base + VPE_OP_MODE_OFFSET);
}
...
static int vpe_probe(struct platform_device *pdev)
{
	...
	legacy_m_msm_sd_register(&vpe_dev->msm_sd);
	legacy_m_msm_cam_copy_v4l2_subdev_fops(&msm_vpe_v4l2_subdev_fops);
...
err_unregister_sd:
	legacy_m_msm_sd_unregister(&vpe_dev->msm_sd);
err_release_mem:
