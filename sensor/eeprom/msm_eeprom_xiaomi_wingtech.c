...
extern int legacy_xiaomi_wingtech_ov13850_eeprom_sensor_writereg(struct msm_eeprom_ctrl_t *s_ctrl,
...
#ifdef HI556_USE_OTP
extern int legacy_xiaomi_tiare_hi556_insensor_read_otp_info(struct msm_eeprom_ctrl_t *e_ctrl);
#endif
#endif
...
int legacy_xiaomi_wingtech_eeprom_match_id(
	struct msm_eeprom_ctrl_t *e_ctrl, struct msm_eeprom_board_info *eb_info)
{
	...
	if (!strcmp(eb_info->eeprom_name, "s5k3l8_ofilm")) {
		...
		if (xiaomi_msm8937_mach_get() == XIAOMI_MSM8937_MACH_RIVA ||
			...
			legacy_xiaomi_wingtech_ov13850_eeprom_sensor_writereg(e_ctrl, 0x0100, 0x00, 1);
...
#ifdef HI556_USE_OTP
		rc = legacy_xiaomi_tiare_hi556_insensor_read_otp_info(e_ctrl);
		if (rc < 0) {
			pr_err("%s legacy_xiaomi_tiare_hi556_insensor_read_otp_info failed!\n",
				   __func__);
			goto power_down;
		}
#endif
...
int legacy_xiaomi_wingtech_sensor_match_id(
...
void legacy_xiaomi_wingtech_sensor_read_fusion_id(
	struct msm_camera_sensor_slave_info *slave_info,
