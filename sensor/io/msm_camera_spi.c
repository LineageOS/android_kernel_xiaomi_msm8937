...
/**
  * legacy_msm_camera_spi_tx_helper() - wrapper for SPI transaction
  * @client:	io client
  ...
  */
int32_t legacy_msm_camera_spi_tx_helper(struct msm_camera_i2c_client *client,
...
int32_t legacy_msm_camera_spi_tx_read(struct msm_camera_i2c_client *client,
	struct msm_camera_spi_inst *inst, uint32_t addr, uint8_t *data,
	uint32_t num_byte, char *tx, char *rx)
{
	int32_t rc = -EINVAL;
...
int32_t legacy_msm_camera_spi_read(struct msm_camera_i2c_client *client,
	...
	enum msm_camera_i2c_data_type data_type)
{
	...
	rc = legacy_msm_camera_spi_tx_read(client,
...
int32_t legacy_msm_camera_spi_read_seq(struct msm_camera_i2c_client *client,
	uint32_t addr, uint8_t *data, uint32_t num_byte)
{
	return legacy_msm_camera_spi_tx_helper(client,
		&client->spi_client->cmd_tbl.read_seq, addr, data, num_byte,
		NULL, NULL);
}

/**
  * legacy_msm_camera_spi_read_seq_l()- function for large SPI reads
  * @client:	io client
  ...
  */
int32_t legacy_msm_camera_spi_read_seq_l(struct msm_camera_i2c_client *client,
...
int32_t legacy_msm_camera_spi_query_id(struct msm_camera_i2c_client *client,
	uint32_t addr, uint8_t *data, uint32_t num_byte)
{
	return legacy_msm_camera_spi_tx_helper(client,
	...
	struct msm_camera_i2c_client *client, uint8_t *status)
{
	...
	return legacy_msm_camera_spi_tx_helper(client, rs, 0, status, 1, NULL, NULL);
}
	...
	struct msm_camera_i2c_client *client)
{
	...
	rc = legacy_msm_camera_spi_tx_helper(client, we, 0, NULL, 0, NULL, NULL);
	if (rc < 0)
		pr_err("%s: write enable failed\n", __func__);
	return rc;
}

int32_t legacy_msm_camera_spi_erase(struct msm_camera_i2c_client *client,
	uint32_t addr, uint32_t size)
{
	...
	for (cur = rounddown(addr, erase_size); cur < end; cur += erase_size) {
		...
		if (rc < 0)
			return rc;
		rc = legacy_msm_camera_spi_tx_helper(client, se, cur, NULL, 0,
			NULL, NULL);
		if (rc < 0) {
...
/**
  ...
  * used outside legacy_msm_camera_spi_write_seq().
  */
...
int32_t legacy_msm_camera_spi_write_seq(struct msm_camera_i2c_client *client,
	uint32_t addr, uint8_t *data, uint32_t num_byte)
{
	struct msm_camera_spi_inst *pg =
		&client->spi_client->cmd_tbl.page_program;
...
int32_t legacy_msm_camera_spi_write(struct msm_camera_i2c_client *client,
...
int32_t legacy_msm_camera_spi_write_table(struct msm_camera_i2c_client *client,
	struct msm_camera_i2c_reg_setting *write_setting)
{
	...
	for (i = 0; i < write_setting->size; i++) {
		...
		rc = legacy_msm_camera_spi_write(client, reg_setting->reg_addr,
...
uint32_t legacy_msm_get_burst_size(struct msm_camera_i2c_reg_array *reg_setting,
...
int32_t legacy_msm_camera_spi_send_burst(struct msm_camera_i2c_client *client,
...
int32_t legacy_msm_camera_spi_write_burst(struct msm_camera_i2c_client *client,
	struct msm_camera_i2c_reg_array *reg_setting, uint32_t reg_size,
	...
	while (k < reg_size) {
		if (reg_setting[k].reg_addr == burst_addr) {
			...
			burst_info.burst_len =
				legacy_msm_get_burst_size(reg_setting, reg_size, k,
					burst_addr);
			SPIDBG("%s burst start = %d, length = %d\n", __func__,
				k, burst_info.burst_len);
			rc = legacy_msm_camera_spi_send_burst(client, reg_setting,
			...
			legacy_msm_camera_spi_write(client, reg_setting[k].reg_addr,
...
int32_t legacy_msm_camera_spi_read_burst(struct msm_camera_i2c_client *client,
