...
static int msm_jpegdma_open(struct file *file)
{
	...
	ret = legacy_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_JPEG,
			CAM_AHB_SVS_VOTE);
	if (ret < 0) {
...
static int jpegdma_probe(struct platform_device *pdev)
{
	...
	ret = legacy_msm_camera_get_regulator_info(pdev, &jpegdma->dma_vdd,
		&jpegdma->num_reg);
	if (ret < 0)
		goto error_get_regulators;

	/* get all the clocks */
	ret = legacy_msm_camera_get_clk_info(pdev, &jpegdma->jpeg_clk_info,
	...
	jpegdma->irq = legacy_msm_camera_get_irq(pdev, "jpeg");
	if (!jpegdma->irq)
	...
	/* register bus client */
	ret = legacy_msm_camera_register_bus_client(pdev,
			jpegdma->bus_client);
	if (ret < 0) {
...
error_m2m_init:
error_hw_get_cap:
	legacy_msm_camera_unregister_bus_client(jpegdma->bus_client);
error_reg_bus:
error_hw_get_irq:
...
error_qos_get:
	legacy_msm_camera_put_clk_info(pdev, &jpegdma->jpeg_clk_info,
		&jpegdma->clk, jpegdma->num_clk);
error_get_clocks:
	legacy_msm_camera_put_regulators(pdev, &jpegdma->dma_vdd,
		jpegdma->num_reg);
...
static int jpegdma_device_remove(struct platform_device *pdev)
{
	...
	legacy_msm_camera_unregister_bus_client(dma->bus_client);
	/* release all the regulators */
	legacy_msm_camera_put_regulators(dma->pdev, &dma->dma_vdd,
		dma->num_reg);
	/* release all the clocks */
	legacy_msm_camera_put_clk_info(dma->pdev, &dma->jpeg_clk_info,
		&dma->clk, dma->num_clk);
