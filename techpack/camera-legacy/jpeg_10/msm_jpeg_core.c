...
int legacy_msm_jpeg_core_reset(struct msm_jpeg_device *pgmn_dev, uint8_t op_mode,
	void *base, int size) {
	unsigned long flags;
	int rc = 0;
	int tm = 500; /*500ms*/
	...
	if (pgmn_dev->core_type == MSM_JPEG_CORE_CODEC)
		legacy_msm_jpeg_hw_reset(base, size);
	else
		legacy_msm_jpeg_hw_reset_dma(base, size);
...
void legacy_msm_jpeg_core_release(struct msm_jpeg_device *pgmn_dev)
{
	int i = 0;
	for (i = 0; i < 2; i++) {
...
void legacy_msm_jpeg_core_init(struct msm_jpeg_device *pgmn_dev)
{
	init_waitqueue_head(&pgmn_dev->reset_wait);
	spin_lock_init(&pgmn_dev->reset_lock);
}

int legacy_msm_jpeg_core_fe_start(struct msm_jpeg_device *pgmn_dev)
{
	legacy_msm_jpeg_hw_fe_start(pgmn_dev->base);
	return 0;
}

/* fetch engine */
int legacy_msm_jpeg_core_fe_buf_update(struct msm_jpeg_device *pgmn_dev,
	struct msm_jpeg_core_buf *buf)
{
	...
	if (pgmn_dev->core_type == MSM_JPEG_CORE_CODEC) {
		rc = legacy_msm_jpeg_hw_pingpong_update(&pgmn_dev->fe_pingpong_buf,
			buf, pgmn_dev->base);
		if (rc < 0)
			return rc;
		legacy_msm_jpeg_hw_fe_mmu_prefetch(buf, pgmn_dev->base,
			pgmn_dev->decode_flag);
	} else {
		rc = legacy_msm_jpegdma_hw_pingpong_update(
			&pgmn_dev->fe_pingpong_buf, buf, pgmn_dev->base);
		if (rc < 0)
			return rc;
		legacy_msm_jpegdma_hw_fe_mmu_prefetch(buf, pgmn_dev->base);
	}

	return rc;
}

void *legacy_msm_jpeg_core_fe_pingpong_irq(int jpeg_irq_status,
	struct msm_jpeg_device *pgmn_dev)
{
	return legacy_msm_jpeg_hw_pingpong_irq(&pgmn_dev->fe_pingpong_buf);
}

/* write engine */
int legacy_msm_jpeg_core_we_buf_update(struct msm_jpeg_device *pgmn_dev,
	struct msm_jpeg_core_buf *buf) {
	...
	if (pgmn_dev->core_type == MSM_JPEG_CORE_CODEC) {
		...
		legacy_msm_jpeg_hw_we_mmu_prefetch(buf, pgmn_dev->base,
			pgmn_dev->decode_flag);
	} else {
		legacy_msm_jpegdma_hw_we_buffer_update(
			&pgmn_dev->we_pingpong_buf.buf[0], 0, pgmn_dev->base);
		legacy_msm_jpegdma_hw_we_mmu_prefetch(buf, pgmn_dev->base);
	}

	return 0;
}

int legacy_msm_jpeg_core_we_buf_reset(struct msm_jpeg_device *pgmn_dev,
...
void *legacy_msm_jpeg_core_we_pingpong_irq(int jpeg_irq_status,
	struct msm_jpeg_device *pgmn_dev)
{
	JPEG_DBG("%s:%d]\n", __func__, __LINE__);

	return legacy_msm_jpeg_hw_pingpong_irq(&pgmn_dev->we_pingpong_buf);
}

void *legacy_msm_jpeg_core_framedone_irq(int jpeg_irq_status,
	struct msm_jpeg_device *pgmn_dev)
{
	...
	buf_p = legacy_msm_jpeg_hw_pingpong_active_buffer(
		&pgmn_dev->we_pingpong_buf);
	if (buf_p && !pgmn_dev->decode_flag) {
		buf_p->framedone_len =
			legacy_msm_jpeg_hw_encode_output_size(pgmn_dev->base);
...
void *legacy_msm_jpeg_core_reset_ack_irq(int jpeg_irq_status,
	struct msm_jpeg_device *pgmn_dev)
{
	/* @todo return the status back to legacy_msm_jpeg_core_reset */
	JPEG_DBG("%s:%d]\n", __func__, __LINE__);
	return NULL;
}

void *legacy_msm_jpeg_core_err_irq(int jpeg_irq_status,
	struct msm_jpeg_device *pgmn_dev)
{
...
void legacy_msm_jpeg_core_return_buffers(struct msm_jpeg_device *pgmn_dev,
	 int jpeg_irq_status)
{
	void *data = NULL;
	data = legacy_msm_jpeg_core_fe_pingpong_irq(jpeg_irq_status,
		pgmn_dev);
	if (msm_jpeg_irq_handler)
		msm_jpeg_irq_handler(MSM_JPEG_HW_MASK_COMP_FE,
			pgmn_dev, data);
	data = legacy_msm_jpeg_core_we_pingpong_irq(jpeg_irq_status,
		pgmn_dev);
	if (msm_jpeg_irq_handler)
...
irqreturn_t legacy_msm_jpeg_core_irq(int irq_num, void *context)
{
	void *data = NULL;
	unsigned long flags;
	int jpeg_irq_status;
	...
	jpeg_irq_status = legacy_msm_jpeg_hw_irq_get_status(pgmn_dev->base);
	...
	if (pgmn_dev->state == MSM_JPEG_IDLE) {
		...
		__LINE__);
		legacy_msm_jpeg_hw_irq_clear(JPEG_IRQ_CLEAR_BMSK,
		...
		if (pgmn_dev->decode_flag)
			legacy_msm_jpeg_decode_status(pgmn_dev->base);
	} else {
		legacy_msm_jpeg_hw_irq_clear(JPEG_IRQ_CLEAR_BMSK,
	...
	if (msm_jpeg_hw_irq_is_frame_done(jpeg_irq_status)) {
		...
		if (msm_jpeg_irq_handler)
			msm_jpeg_irq_handler(MSM_JPEG_HW_MASK_COMP_FE,
				context, data);
		data = legacy_msm_jpeg_core_framedone_irq(jpeg_irq_status,
			pgmn_dev);
		if (msm_jpeg_irq_handler)
	...
	if (msm_jpeg_hw_irq_is_reset_ack(jpeg_irq_status)) {
		data = legacy_msm_jpeg_core_reset_ack_irq(jpeg_irq_status,
			pgmn_dev);
		spin_lock_irqsave(&pgmn_dev->reset_lock, flags);
	...
	if (msm_jpeg_hw_irq_is_err(jpeg_irq_status)) {
		if (pgmn_dev->state != MSM_JPEG_EXECUTING) {
			...
			if (pgmn_dev->decode_flag)
				legacy_msm_jpeg_decode_status(pgmn_dev->base);
			legacy_msm_jpeg_core_return_buffers(pgmn_dev, jpeg_irq_status);
			data = legacy_msm_jpeg_core_err_irq(jpeg_irq_status, pgmn_dev);
			if (msm_jpeg_irq_handler) {
...
irqreturn_t legacy_msm_jpegdma_core_irq(int irq_num, void *context)
{
	void *data = NULL;
	unsigned long flags;
	int jpeg_irq_status;
	...
	jpeg_irq_status = legacy_msm_jpegdma_hw_irq_get_status(pgmn_dev->base);
	...
	if (pgmn_dev->state == MSM_JPEG_IDLE) {
		...
		JPEG_DBG_HIGH("%s %d ] Ignoring the Error", __func__,
		__LINE__);
		legacy_msm_jpegdma_hw_irq_clear(JPEGDMA_IRQ_CLEAR_BMSK,
...
void legacy_msm_jpeg_core_irq_install(int (*irq_handler) (int, void *, void *))
{
	msm_jpeg_irq_handler = irq_handler;
}

void legacy_msm_jpeg_core_irq_remove(void)
{
