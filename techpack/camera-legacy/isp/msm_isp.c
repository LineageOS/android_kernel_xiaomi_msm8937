...
struct msm_isp_ub_info legacy_ub_info;
	...
	size_t t_size_t, loff_t *t_loff_t)
{
	...
	legacy_msm_isp_util_get_bandwidth_stats(vfe_dev, stats);
	ptr = (uint64_t *)(stats);
	...
	size_t t_size_t, loff_t *t_loff_t)
{
	...
	struct msm_isp_ub_info *legacy_ub_info = vfe_dev->legacy_ub_info;
	...
	snprintf(line_buffer, sizeof(line_buffer),
		...
		"num_wm = %d\n"
		"wm_ub = %d\n",
		legacy_ub_info->policy, legacy_ub_info->num_wm, legacy_ub_info->wm_ub);
	strlcat(out_buffer, line_buffer,
	    sizeof(ub_info_buffer));
	for (i = 0; i < legacy_ub_info->num_wm; i++) {
		snprintf(line_buffer, sizeof(line_buffer),
			"data[%d] = 0x%x, addr[%d] = 0x%llx\n",
			i, legacy_ub_info->data[i], i, legacy_ub_info->addr[i]);
		strlcat(out_buffer, line_buffer,
			sizeof(ub_info_buffer));
	}
	...
	const char *t_char, size_t t_size_t, loff_t *t_loff_t)
{
	...
	memset(legacy_ub_info, 0, sizeof(struct msm_isp_ub_info));
	...
	struct msm_isp_bw_req_info *isp_req_hist)
{
	...
	if (!debugfs_create_file("legacy_ub_info", S_IRUGO | S_IWUSR,
...
void legacy_msm_isp_update_req_history(uint32_t client, uint64_t ab,
				 uint64_t ib,
...
void legacy_msm_isp_update_last_overflow_ab_ib(struct vfe_device *vfe_dev)
{
	spin_lock(&req_history_lock);
...
int legacy_vfe_hw_probe(struct platform_device *pdev)
{
	struct vfe_device *vfe_dev;
	...
	vfe_dev->legacy_ub_info = kzalloc(sizeof(struct msm_isp_ub_info), GFP_KERNEL);
	if (!vfe_dev->legacy_ub_info) {
		pr_err("%s: no enough memory\n", __func__);
		rc = -ENOMEM;
		goto probe_fail2;
	}
	...
	tasklet_init(&vfe_dev->vfe_tasklet,
		legacy_msm_isp_do_tasklet, (unsigned long)vfe_dev);
	...
	rc = legacy_msm_sd_register(&vfe_dev->subdev);
	if (rc != 0) {
		pr_err("%s: legacy_msm_sd_register error = %d\n", __func__, rc);
		goto probe_fail3;
	}
	legacy_msm_cam_copy_v4l2_subdev_fops(&msm_isp_v4l2_fops);
	...
	rc = legacy_msm_isp_create_isp_buf_mgr(vfe_dev->buf_mgr,
