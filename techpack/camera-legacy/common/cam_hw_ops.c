...
int legacy_get_vector_index(char *name)
{
	int i = 0, rc = -1;
...
int legacy_cam_ahb_clk_init(struct platform_device *pdev)
{
	...
	*data.pbus_data = (struct msm_bus_scale_pdata) {
	...
	index = legacy_get_vector_index("suspend");
	if (index < 0) {
...
EXPORT_SYMBOL(legacy_cam_ahb_clk_init);

int legacy_cam_consolidate_ahb_vote(enum cam_ahb_clk_client id,
	enum cam_ahb_clk_vote vote)
{
	int i = 0;
	u32 max = 0;
...
int legacy_cam_config_ahb_clk(struct device *dev, unsigned long freq,
	enum cam_ahb_clk_client id, enum cam_ahb_clk_vote vote)
{
	...
	rc = legacy_cam_consolidate_ahb_vote(id, dyn_vote);
	if (rc < 0) {
...
EXPORT_SYMBOL(legacy_cam_config_ahb_clk);
