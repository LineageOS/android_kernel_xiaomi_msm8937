...
int legacy_cam_smmu_query_vaddr_in_range(int handle,
...
EXPORT_SYMBOL(legacy_cam_smmu_query_vaddr_in_range);
...
void legacy_cam_smmu_reg_client_page_fault_handler(int handle,
...
void legacy_cam_smmu_reset_iommu_table(enum cam_smmu_init_dir ops)
{
	unsigned int i;
	int j = 0;
	for (i = 0; i < iommu_cb_set.cb_num; i++) {
...
int legacy_cam_smmu_get_handle(char *identifier, int *handle_ptr)
{
	int ret = 0;
...
EXPORT_SYMBOL(legacy_cam_smmu_get_handle);

int legacy_cam_smmu_ops(int handle, enum cam_smmu_ops_param ops)
{
	int ret = 0, idx;
...
EXPORT_SYMBOL(legacy_cam_smmu_ops);
...
int legacy_cam_smmu_get_phy_addr_scratch(int handle,
				  enum cam_smmu_map_dir dir,
...
int legacy_cam_smmu_put_phy_addr_scratch(int handle,
				  dma_addr_t paddr)
{
	int idx;
	int rc = -1;
...
int legacy_cam_smmu_get_phy_addr(int handle, int ion_fd,
...
EXPORT_SYMBOL(legacy_cam_smmu_get_phy_addr);

int legacy_cam_smmu_put_phy_addr(int handle, int ion_fd)
{
	int idx, rc;
	struct cam_dma_buff_info *mapping_info;
...
EXPORT_SYMBOL(legacy_cam_smmu_put_phy_addr);

int legacy_cam_smmu_destroy_handle(int handle)
{
	int idx;
...
EXPORT_SYMBOL(legacy_cam_smmu_destroy_handle);

/*This function can only be called after smmu driver probe*/
int legacy_cam_smmu_get_num_of_clients(void)
{
...
static int cam_alloc_smmu_context_banks(struct device *dev)
{
	...
	legacy_cam_smmu_reset_iommu_table(CAM_SMMU_TABLE_INIT);
...
static int cam_smmu_remove(struct platform_device *pdev)
{
	...
	legacy_cam_smmu_reset_iommu_table(CAM_SMMU_TABLE_DEINIT);
