...
{
	uint32_t val;
	val = legacy_msm_camera_io_r(mux_device->ctl_base);
	if (*mode == MODE_DUAL) {
		legacy_msm_camera_io_w(val | 0x3, mux_device->ctl_base);
	} else if (*mode == MODE_L) {
		legacy_msm_camera_io_w(((val | 0x2) & ~(0x1)), mux_device->ctl_base);
		val = legacy_msm_camera_io_r(mux_device->ctl_base);
		CDBG("the camio mode config left value is %d\n", val);
	} else {
		legacy_msm_camera_io_w(((val | 0x1) & ~(0x2)), mux_device->ctl_base);
		val = legacy_msm_camera_io_r(mux_device->ctl_base);
...
static int msm_i2c_mux_init(struct i2c_mux_device *mux_device)
{
	...
	if (mux_device->use_count == 0) {
		val = legacy_msm_camera_io_r(mux_device->rw_base);
		legacy_msm_camera_io_w((val | 0x200), mux_device->rw_base);
	}
...
static int i2c_mux_probe(struct platform_device *pdev)
{
	...
	mux_device->ctl_base = legacy_msm_camera_get_reg_base(pdev,
		"i2c_mux_ctl", true);
	...
	mux_device->rw_base = legacy_msm_camera_get_reg_base(pdev, "i2c_mux_rw", true);
...
rw_base_failed:
	legacy_msm_camera_put_reg_base(pdev, mux_device->ctl_base,
		"i2c_mux_ctl", true);
ctl_base_failed:
...
static int i2c_mux_remove(struct platform_device *pdev)
{
	...
	legacy_msm_camera_put_reg_base(pdev, mux_device->rw_base, "i2c_mux_ctl", true);
	legacy_msm_camera_put_reg_base(pdev, mux_device->ctl_base, "i2c_mux_rw", true);
}
