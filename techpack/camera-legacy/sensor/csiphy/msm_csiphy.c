	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	legacy_msm_camera_io_w(csiphy_dev->ctrl_reg->csiphy_3ph_reg.
	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	legacy_msm_camera_io_w(lane_enable,
		csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
	...
	while (lane_mask & 0x7) {
		...
		legacy_msm_camera_io_w(((csiphy_params->settle_cnt >> 8) & 0xff),
			csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
			mipi_csiphy_3ph_lnn_ctrl2.addr + 0x200*i);
		legacy_msm_camera_io_w((csiphy_params->settle_cnt & 0xff),
			csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
	...
	if (csiphy_params->combo_mode == 1) {
		legacy_msm_camera_io_w(0x2,
			csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
			mipi_csiphy_3ph_cmn_ctrl7.addr);
	} else {
		legacy_msm_camera_io_w(0x6,
			csiphybase + csiphy_dev->ctrl_reg->csiphy_3ph_reg.
			mipi_csiphy_3ph_cmn_ctrl7.addr);
	}
	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	lane_enable = legacy_msm_camera_io_r(csiphybase +
	...
	for (i = 0, mask = 0x1; i < MAX_LANES; i++) {
		...
		legacy_msm_camera_io_w(val, csiphybase +
			csiphy_dev->ctrl_reg->csiphy_3ph_reg.
	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	clk_rate = legacy_msm_camera_clk_set_rate(&csiphy_dev->pdev->dev,
	...
	if (csiphy_dev->hw_version >= CSIPHY_VERSION_V30) {
		val = legacy_msm_camera_io_r(csiphy_dev->clk_mux_base);
		...
		legacy_msm_camera_io_w(val, csiphy_dev->clk_mux_base);
	...
	if (csiphy_dev->csiphy_3phase == CSI_3PHASE_HW) {
		if (csiphy_dev->csi_3phase == 1) {
			rc = legacy_msm_camera_clk_enable(&csiphy_dev->pdev->dev,
	...
	legacy_msm_camera_io_w(0x1, csiphybase + csiphy_dev->ctrl_reg->
	...
	if (csiphy_dev->hw_version < CSIPHY_VERSION_V30) {
		val = 0x3;
		legacy_msm_camera_io_w((lane_mask << 2) | val,
				csiphybase +
				csiphy_dev->ctrl_reg->
				csiphy_reg.mipi_csiphy_glbl_pwr_cfg_addr);
		legacy_msm_camera_io_w(0x10, csiphybase +
			csiphy_dev->ctrl_reg->csiphy_reg.
			mipi_csiphy_lnck_cfg2_addr);
		legacy_msm_camera_io_w(csiphy_params->settle_cnt,
			csiphybase +
			csiphy_dev->ctrl_reg->csiphy_reg.
			mipi_csiphy_lnck_cfg3_addr);
		legacy_msm_camera_io_w(0x24,
			csiphybase + csiphy_dev->ctrl_reg->
		...
		val = 0x1;
		legacy_msm_camera_io_w((lane_mask << 1) | val,
				csiphybase +
				csiphy_dev->ctrl_reg->
				csiphy_reg.mipi_csiphy_glbl_pwr_cfg_addr);
		legacy_msm_camera_io_w(csiphy_params->combo_mode <<
			csiphy_dev->ctrl_reg->csiphy_reg.
	...
	while (lane_mask & 0x1f) {
		...
		if (csiphy_dev->is_3_1_20nm_hw == 1) {
			...
			legacy_msm_camera_io_w(lane_val, csiphybase +
				csiphy_dev->ctrl_reg->csiphy_reg.
				mipi_csiphy_lnn_misc1_addr + 0x40*j);
			legacy_msm_camera_io_w(0x17, csiphybase +
				csiphy_dev->ctrl_reg->csiphy_reg.
...
void legacy_msm_csiphy_disable_irq(
	struct csiphy_device *csiphy_dev)
{
	void __iomem *csiphybase;
...
static irqreturn_t msm_csiphy_irq(int irq_num, void *data)
{
	...
	if (csiphy_dev->csiphy_sof_debug == SOF_DEBUG_ENABLE) {
		if (csiphy_dev->csiphy_sof_debug_count < CSIPHY_SOF_DEBUG_COUNT)
			...
			legacy_msm_csiphy_disable_irq(csiphy_dev);
			return IRQ_HANDLED;
		}
	}

	for (i = 0; i < csiphy_dev->num_irq_registers; i++) {
		irq = legacy_msm_camera_io_r(
			csiphy_dev->base +
			csiphy_dev->ctrl_reg->csiphy_reg.
			mipi_csiphy_interrupt_status0_addr + 0x4*i);
		legacy_msm_camera_io_w(irq,
			csiphy_dev->base +
			csiphy_dev->ctrl_reg->csiphy_reg.
	...
	legacy_msm_camera_io_w(0x1, csiphy_dev->base +
		csiphy_dev->ctrl_reg->
		csiphy_reg.mipi_csiphy_glbl_irq_cmd_addr);
	legacy_msm_camera_io_w(0x0, csiphy_dev->base +
		csiphy_dev->ctrl_reg->
		csiphy_reg.mipi_csiphy_glbl_irq_cmd_addr);
	return IRQ_HANDLED;
}
...
static int msm_csiphy_init(struct csiphy_device *csiphy_dev)
{
	...
	rc = legacy_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_CSIPHY,
			CAM_AHB_SVS_VOTE);
	if (rc < 0) {
		csiphy_dev->ref_count--;
	...
	rc = legacy_msm_camera_config_vreg(&csiphy_dev->pdev->dev,
	...
	rc = legacy_msm_camera_enable_vreg(&csiphy_dev->pdev->dev,
		csiphy_dev->csiphy_vreg,
		csiphy_dev->regulator_count, NULL, 0,
		&csiphy_dev->csiphy_reg_ptr[0], 1);
	if (rc < 0) {
	...
	rc = legacy_msm_camera_enable_irq(csiphy_dev->irq, true);
	if (rc < 0)
	...
	if (csiphy_dev->hw_dts_version == CSIPHY_VERSION_V30)
		csiphy_dev->hw_version =
			legacy_msm_camera_io_r(csiphy_dev->base +
				 csiphy_dev->ctrl_reg->
...
csiphy_enable_clk_fail:
	legacy_msm_camera_enable_vreg(&csiphy_dev->pdev->dev,
		csiphy_dev->csiphy_vreg,
		csiphy_dev->regulator_count, NULL, 0,
		&csiphy_dev->csiphy_reg_ptr[0], 0);
top_vreg_enable_failed:
	legacy_msm_camera_config_vreg(&csiphy_dev->pdev->dev,
		csiphy_dev->csiphy_vreg,
		csiphy_dev->regulator_count, NULL, 0,
		&csiphy_dev->csiphy_reg_ptr[0], 0);
csiphy_vreg_config_fail:
	if (legacy_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_CSIPHY,
...
static int msm_csiphy_release(struct csiphy_device *csiphy_dev, void *arg)
{
	...
	rc = legacy_msm_camera_enable_irq(csiphy_dev->irq, false);

	legacy_msm_camera_clk_enable(&csiphy_dev->pdev->dev,
		csiphy_dev->csiphy_clk_info, csiphy_dev->csiphy_clk,
	...
	struct platform_device *pdev)
{
	...
	rc = legacy_msm_camera_get_clk_info(csiphy_dev->pdev,
	...
	return rc;
MAX_CLK_ERROR:
	legacy_msm_camera_put_clk_info(csiphy_dev->pdev,
		&csiphy_dev->csiphy_all_clk_info,
		&csiphy_dev->csiphy_all_clk,
		csiphy_dev->num_all_clk);
...
static int csiphy_probe(struct platform_device *pdev)
{
	...
	legacy_msm_sd_register(&new_csiphy_dev->msm_sd);
	...
	if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v2.0")) {
		new_csiphy_dev->ctrl_reg->csiphy_reg = legacy_csiphy_v2_0;
		new_csiphy_dev->hw_dts_version = CSIPHY_VERSION_V20;
	} else if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v2.2")) {
		new_csiphy_dev->ctrl_reg->csiphy_reg = legacy_csiphy_v2_2;
		new_csiphy_dev->hw_dts_version = CSIPHY_VERSION_V22;
	} else if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v3.0")) {
		new_csiphy_dev->ctrl_reg->csiphy_reg = legacy_csiphy_v3_0;
		new_csiphy_dev->hw_dts_version = CSIPHY_VERSION_V30;
	} else if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v3.1")) {
		new_csiphy_dev->ctrl_reg->csiphy_reg = legacy_csiphy_v3_1;
		...
		"qcom,csiphy-v3.2")) {
		new_csiphy_dev->ctrl_reg->csiphy_reg = legacy_csiphy_v3_2;
		new_csiphy_dev->hw_dts_version = CSIPHY_VERSION_V32;
	} else if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v3.4.2")) {
		new_csiphy_dev->ctrl_reg->csiphy_3ph_reg = legacy_csiphy_v3_4_2_3ph;
		new_csiphy_dev->ctrl_reg->csiphy_reg = legacy_csiphy_v3_4_2;
		new_csiphy_dev->hw_dts_version = CSIPHY_VERSION_V342;
		new_csiphy_dev->csiphy_3phase = CSI_3PHASE_HW;
	} else if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v3.4.2.1")) {
		new_csiphy_dev->ctrl_reg->csiphy_3ph_reg = legacy_csiphy_v3_4_2_1_3ph;
		new_csiphy_dev->ctrl_reg->csiphy_reg = legacy_csiphy_v3_4_2_1;
		new_csiphy_dev->hw_dts_version = CSIPHY_VERSION_V342_1;
		new_csiphy_dev->csiphy_3phase = CSI_3PHASE_HW;
	} else if (of_device_is_compatible(new_csiphy_dev->pdev->dev.of_node,
		"qcom,csiphy-v3.5")) {
		new_csiphy_dev->ctrl_reg->csiphy_3ph_reg = legacy_csiphy_v3_5_3ph;
		new_csiphy_dev->ctrl_reg->csiphy_reg = legacy_csiphy_v3_5;
	...
	new_csiphy_dev->base = legacy_msm_camera_get_reg_base(pdev, "csiphy", true);
	...
	if (new_csiphy_dev->hw_dts_version >= CSIPHY_VERSION_V30) {
		new_csiphy_dev->clk_mux_base = legacy_msm_camera_get_reg_base(pdev,
					"csiphy_clk_mux", true);
	...
	new_csiphy_dev->irq = legacy_msm_camera_get_irq(pdev, "csiphy");
	if (!new_csiphy_dev->irq) {
		pr_err("%s: no irq resource?\n", __func__);
		rc = -ENODEV;
		goto csiphy_no_irq_resource;
	}
	rc = legacy_msm_camera_register_irq(pdev, new_csiphy_dev->irq,
	...
	legacy_msm_camera_enable_irq(new_csiphy_dev->irq, false);

	legacy_msm_cam_copy_v4l2_subdev_fops(&msm_csiphy_v4l2_subdev_fops);
#ifdef CONFIG_COMPAT
...
csiphy_no_irq_resource:
	if (new_csiphy_dev->hw_dts_version >= CSIPHY_VERSION_V30) {
		legacy_msm_camera_put_reg_base(pdev, new_csiphy_dev->clk_mux_base,
			"csiphy_clk_mux", true);
	}
csiphy_no_mux_resource:
	legacy_msm_camera_put_reg_base(pdev, new_csiphy_dev->base, "csiphy", true);
csiphy_no_resource:
...
static int msm_csiphy_exit(struct platform_device *pdev)
{
	...
	legacy_msm_camera_put_clk_info(pdev,
		&csiphy_dev->csiphy_all_clk_info,
		&csiphy_dev->csiphy_all_clk,
		csiphy_dev->num_all_clk);

	legacy_msm_camera_put_reg_base(pdev, csiphy_dev->base, "csiphy", true);
	if (csiphy_dev->hw_dts_version >= CSIPHY_VERSION_V30) {
		legacy_msm_camera_put_reg_base(pdev, csiphy_dev->clk_mux_base,
			"csiphy_clk_mux", true);
	}
	kfree(csiphy_dev);
	return 0;
