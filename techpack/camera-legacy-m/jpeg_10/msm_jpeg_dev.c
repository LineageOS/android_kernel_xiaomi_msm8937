...
static int msm_jpeg_open(struct inode *inode, struct file *filp)
{
	...
	rc = legacy_m___msm_jpeg_open(pgmn_dev);
...
static int msm_jpeg_release(struct inode *inode, struct file *filp)
{
	...
	rc = legacy_m___msm_jpeg_release(pgmn_dev);
	...
	unsigned long arg)
{
	...
	rc = legacy_m___msm_jpeg_compat_ioctl(pgmn_dev, cmd, arg);
	...
	unsigned long arg)
{
	...
	rc = legacy_m___msm_jpeg_ioctl(pgmn_dev, cmd, arg);
...
int legacy_m_msm_jpeg_subdev_init(struct v4l2_subdev *jpeg_sd)
{
	int rc;
	...
	unsigned int cmd, void *arg)
{
	...
	JPEG_DBG("%s: Calling legacy_m___msm_jpeg_ioctl\n", __func__);

	rc = legacy_m___msm_jpeg_ioctl(pgmn_dev, cmd, (unsigned long)arg);
	pr_debug("%s: X\n", __func__);
	return rc;
}

void legacy_m_msm_jpeg_subdev_release(struct v4l2_subdev *jpeg_sd)
{
	int rc;
	struct msm_jpeg_device *pgmn_dev =
		(struct msm_jpeg_device *)jpeg_sd->host_priv;
...
static int msm_jpeg_init_dev(struct platform_device *pdev)
{
	...
	rc = legacy_m___msm_jpeg_init(msm_jpeg_device_p);
	if (rc < -1) {
...
fail_1:
	legacy_m___msm_jpeg_exit(msm_jpeg_device_p);
	return rc;
...
static void msm_jpeg_exit(struct msm_jpeg_device *msm_jpeg_device_p)
{
	...
	class_destroy(msm_jpeg_device_p->msm_jpeg_class);
	unregister_chrdev_region(msm_jpeg_device_p->msm_jpeg_devno, 1);
	legacy_m_cam_smmu_destroy_handle(msm_jpeg_device_p->iommu_hdl);
