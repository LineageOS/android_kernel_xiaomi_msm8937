		...
		enum msm_isp_irq_operation oper)
{
	...
	legacy_m_msm_camera_io_w_mb(vfe_dev->irq0_mask, vfe_dev->vfe_base + 0x28);
	legacy_m_msm_camera_io_w_mb(vfe_dev->irq1_mask, vfe_dev->vfe_base + 0x2C);
}
			...
			if (qos_parms->settings) {
				...
				if (rc < 0) {
					...
					for (i = 0; i < qos_entries; i++)
						legacy_m_msm_camera_io_w(qos_settings[i],
							vfebase + qos_regs[i]);
				}
			}
		}
			...
			if (ds_parms->settings) {
				...
				if (rc < 0) {
					...
					for (i = 0; i < ds_entries; i++)
						legacy_m_msm_camera_io_w(ds_settings[i],
							vfebase + ds_regs[i]);
				}
			}
		}
...
static void msm_vfe40_init_hardware_reg(struct vfe_device *vfe_dev)
{
	...
	/* BUS_CFG */
	legacy_m_msm_camera_io_w(0x10000001, vfe_dev->vfe_base + 0x50);
	msm_vfe40_config_irq(vfe_dev, 0x800000E0, 0xFEFFFF7E,
			MSM_ISP_IRQ_ENABLE);
	legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x30);
	legacy_m_msm_camera_io_w_mb(0xFEFFFFFF, vfe_dev->vfe_base + 0x34);
	legacy_m_msm_camera_io_w(1, vfe_dev->vfe_base + 0x24);

	legacy_m_msm_camera_io_w(0, vfe_dev->vfe_base + 0x30);
	legacy_m_msm_camera_io_w_mb(0, vfe_dev->vfe_base + 0x34);
	legacy_m_msm_camera_io_w(1, vfe_dev->vfe_base + 0x24);
}
...
static void msm_vfe40_clear_status_reg(struct vfe_device *vfe_dev)
{
	...
	msm_vfe40_config_irq(vfe_dev, (1 << 31), 0,
			MSM_ISP_IRQ_SET);
	legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x30);
	legacy_m_msm_camera_io_w_mb(0xFFFFFFFF, vfe_dev->vfe_base + 0x34);
	legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x24);
}
	...
	uint32_t irq_status0, uint32_t irq_status1)
{
	if (irq_status1 & (1 << 8)) {
		...
		legacy_m_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x2C0);
	}
}
...
static void msm_vfe40_process_error_status(struct vfe_device *vfe_dev)
{
	...
	if (error_status1 & (1 << 0)) {
		...
		legacy_m_msm_camera_io_dump(vfe_dev->vfe_base + 0x2F4, 0x30, 1);
	}
	...
	if ((error_status1 >> 9) & 0x7FFF)
		legacy_m_msm_isp_update_last_overflow_ab_ib(vfe_dev);
}

static void msm_vfe40_read_irq_status(struct vfe_device *vfe_dev,
	uint32_t *irq_status0, uint32_t *irq_status1)
{
	*irq_status0 = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x38);
	*irq_status1 = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x3C);
	/*
	 * Ignore composite 2/3 irq which is used for dual VFE only
	 */
	if (*irq_status0 & 0x6000000)
		*irq_status0 &= ~(0x18000000);
	legacy_m_msm_camera_io_w(*irq_status0, vfe_dev->vfe_base + 0x30);
	legacy_m_msm_camera_io_w(*irq_status1, vfe_dev->vfe_base + 0x34);
	legacy_m_msm_camera_io_w_mb(1, vfe_dev->vfe_base + 0x24);
	...
	*irq_status1 &= vfe_dev->irq1_mask;
	...
	if (*irq_status1 & (1 << 0)) {
		vfe_dev->error_info.camif_status =
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x31C);
		msm_vfe40_config_irq(vfe_dev, 0, (1 << 0), MSM_ISP_IRQ_DISABLE);
	}

	if (*irq_status1 & (1 << 7))
		vfe_dev->error_info.violation_status |=
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x48);
	...
	uint32_t first_start, uint32_t blocking_call)
{
	...
	if (first_start) {
		legacy_m_msm_camera_io_w_mb(0x1FF, vfe_dev->vfe_base + 0xC);
	} else {
		legacy_m_msm_camera_io_w_mb(0x1EF, vfe_dev->vfe_base + 0xC);
		legacy_m_msm_camera_io_w(0x7FFFFFFF, vfe_dev->vfe_base + 0x30);
		legacy_m_msm_camera_io_w(0xFEFFFEFF, vfe_dev->vfe_base + 0x34);
		legacy_m_msm_camera_io_w(0x1, vfe_dev->vfe_base + 0x24);
...
{
	legacy_m_msm_camera_io_w_mb(reload_mask, vfe_base + 0x4C);
}
	...
	uint8_t wm_idx, uint8_t enable)
{
	...
	/* Change CGC override */
	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x974);
	if (enable)
		val |= (1 << wm_idx);
	else
		val &= ~(1 << wm_idx);
	legacy_m_msm_camera_io_w_mb(val, vfe_dev->vfe_base + 0x974);
}
...
{
	uint32_t val;
	val = legacy_m_msm_camera_io_r(vfe_base + VFE40_WM_BASE(wm_idx));
	if (enable)
		val |= 0x1;
	else
		val &= ~0x1;
	...
	struct msm_vfe_axi_stream *stream_info)
{
	...
	comp_mask = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x40);
	comp_mask &= ~(0x7F << (comp_mask_index * 8));
	comp_mask |= (axi_data->composite_info[comp_mask_index].
		stream_composite_mask << (comp_mask_index * 8));

	legacy_m_msm_camera_io_w(comp_mask, vfe_dev->vfe_base + 0x40);
	...
	uint32_t framedrop_period)
{
	...
	legacy_m_msm_camera_io_w_mb(0x1, vfe_base + 0x378);
}
	...
	enum msm_vfe_axi_stream_src stream_src, uint32_t io_format)
{
	...
	io_format_reg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x54);
	...
	legacy_m_msm_camera_io_w(io_format_reg, vfe_dev->vfe_base + 0x54);
	return 0;
}
	...
	void *arg)
{
	...
	legacy_m_msm_camera_io_w(mapped_info.paddr, vfe_dev->vfe_base + 0x228);
	legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x378);

	legacy_m_msm_camera_io_w_mb(0x10000, vfe_dev->vfe_base + 0x4C);
	legacy_m_msm_camera_io_w_mb(0x20000, vfe_dev->vfe_base + 0x4C);
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	legacy_m_msm_camera_io_w((fe_cfg->fetch_height - 1),
	...
	legacy_m_msm_camera_io_w((x_size_word - 1) << 16, vfe_dev->vfe_base + 0x23C);

	x_size_word = legacy_m_msm_isp_cal_word_per_line(
		vfe_dev->axi_data.src_info[VFE_PIX_0].input_format,
		fe_cfg->fetch_width);

	temp = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x1C);
	temp |= 2 << 16 | pix_cfg->pixel_pattern;
	legacy_m_msm_camera_io_w(temp, vfe_dev->vfe_base + 0x1C);

	if (vfe_dev->vfe_hw_version == VFE40_8953_VERSION) {
		legacy_m_msm_camera_io_w(x_size_word  << 17 |
			(fe_cfg->buf_height-1) << 4 |
			VFE40_FETCH_BURST_LEN,
			vfe_dev->vfe_base + 0x240);
		legacy_m_msm_camera_io_w(0 << 29 | 2 << 26 |
		...
		legacy_m_msm_camera_io_w(x_size_word  << 16 |
			...
			VFE40_FETCH_BURST_LEN,
			vfe_dev->vfe_base + 0x240);
		legacy_m_msm_camera_io_w(0 << 28 | 2 << 25 |
	...
	legacy_m_msm_camera_io_w(main_unpack_pattern,
		vfe_dev->vfe_base + 0x248);
	legacy_m_msm_camera_io_w(0xF, vfe_dev->vfe_base + 0x264);
		...
		struct msm_vfe_testgen_cfg *testgen_cfg)
{
	...
	legacy_m_msm_camera_io_w(testgen_cfg->h_blank, vfe_dev->vfe_base + 0x958);
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	legacy_m_msm_camera_io_w(pix_cfg->input_mux << 16 | pix_cfg->pixel_pattern,
	...
	legacy_m_msm_camera_io_w(camif_cfg->lines_per_frame << 16 |
	...
	if (subsample_period && subsample_pattern) {
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x2F8);
		val &= 0xFFE0FFFF;
		val = (subsample_period - 1) << 16;
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x2F8);
		ISP_DBG("%s:camif PERIOD %x PATTERN %x\n",
			__func__,  subsample_period, subsample_pattern);

		val = subsample_pattern;
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x314);
	} else {
		legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x314);
	}
	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x2E8);
	val |= camif_cfg->camif_input;
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x2E8);
	...
	if (subsample_cfg->pixel_skip || subsample_cfg->line_skip) {
		...
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x54);
		...
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x54);
	}
}
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	core_cfg =  legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x1C);
	core_cfg &= 0xFFFCFFFF;

	switch (pix_cfg->input_mux) {
	case CAMIF:
		core_cfg |= 0x0 << 16;
		legacy_m_msm_camera_io_w_mb(core_cfg, vfe_dev->vfe_base + 0x1C);
	...
	case TESTGEN:
		...
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x974);
		val |= (1 << 31);
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x974);
	...
	enum msm_isp_camif_update_state update_state)
{
	...
	if (update_state == ENABLE_CAMIF) {
		legacy_m_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x30);
		legacy_m_msm_camera_io_w_mb(0x81, vfe_dev->vfe_base + 0x34);
		legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x24);
		msm_vfe40_config_irq(vfe_dev, 0xF7, 0x81,
				MSM_ISP_IRQ_ENABLE);
		legacy_m_msm_camera_io_w_mb(0x140000, vfe_dev->vfe_base + 0x318);
		...
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			legacy_m_msm_camera_io_w(1, vfe_dev->vfe_base + 0x93C);
		legacy_m_msm_camera_io_w_mb(0x4, vfe_dev->vfe_base + 0x2F4);
		legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x2F4);
		...
		/* testgen OFF*/
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			legacy_m_msm_camera_io_w(1 << 1, vfe_dev->vfe_base + 0x93C);
	}
}
	...
	enum msm_vfe_input_src input_src)
{
	...
	legacy_m_msm_camera_io_w(rdi_reg_cfg,
		vfe_dev->vfe_base + VFE40_RDI_BASE(0));
	...
	uint8_t plane_idx)
{
	...
	if (!stream_info->frame_based) {
		legacy_m_msm_camera_io_w(0x0, vfe_dev->vfe_base + wm_base);
		/*WR_IMAGE_SIZE*/
		val =
			((legacy_m_msm_isp_cal_word_per_line(
		...
		val =
			...
			burst_len;
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x18);
	} else {
		legacy_m_msm_camera_io_w(0x2, vfe_dev->vfe_base + wm_base);
		val =
			legacy_m_msm_isp_cal_word_per_line(stream_info->output_format,
			stream_info->plane_cfg[
	...
	struct msm_vfe_axi_stream *stream_info, uint8_t plane_idx)
{
	...
	uint32_t wm_base = VFE40_WM_BASE(stream_info->wm[plane_idx]);
	/*WR_ADDR_CFG*/
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0xC);
	/*WR_IMAGE_SIZE*/
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x14);
	/*WR_BUFFER_CFG*/
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x18);
	...
	uint8_t plane_idx)
{
	...
	xbar_reg_cfg =
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + VFE40_XBAR_BASE(wm));
	...
	struct vfe_device *vfe_dev)
{
	...
	if (vfe_dev->vfe_hw_version == VFE40_8916_VERSION) {
		vfe_dev->legacy_m_ub_info->wm_ub = VFE40_TOTAL_WM_UB_8916;
		total_wm_ub = VFE40_TOTAL_WM_UB_8916;
	} else {
		vfe_dev->legacy_m_ub_info->wm_ub = VFE40_TOTAL_WM_UB;
		total_wm_ub = VFE40_TOTAL_WM_UB;
	}
	vfe_dev->legacy_m_ub_info->num_wm = axi_data->hw_info->num_wm;
	...
	for (i = 0; i < axi_data->hw_info->num_wm; i++) {
		if (axi_data->free_wm[i]) {
			...
			vfe_dev->legacy_m_ub_info->data[i] =
				ub_offset << 16 | (wm_ub_size - 1);
			vfe_dev->legacy_m_ub_info->addr[i] =  VFE40_WM_BASE(i) + 0x10;
			ub_offset += wm_ub_size;
		} else {
			legacy_m_msm_camera_io_w(0,
				vfe_dev->vfe_base + VFE40_WM_BASE(i) + 0x10);
			vfe_dev->legacy_m_ub_info->data[i] = 0;
			vfe_dev->legacy_m_ub_info->addr[i] = VFE40_WM_BASE(i) + 0x10;
		}
	}
}
	...
	struct vfe_device *vfe_dev)
{
	...
	if (vfe_dev->vfe_hw_version == VFE40_8916_VERSION ||
		...
		vfe_dev->legacy_m_ub_info->wm_ub = VFE40_EQUAL_SLICE_UB_8916;
		equal_slice_ub = VFE40_EQUAL_SLICE_UB_8916;
	} else {
		vfe_dev->legacy_m_ub_info->wm_ub = VFE40_EQUAL_SLICE_UB;
		equal_slice_ub = VFE40_EQUAL_SLICE_UB;
	}

	vfe_dev->legacy_m_ub_info->num_wm = axi_data->hw_info->num_wm;
	for (i = 0; i < axi_data->hw_info->num_wm; i++) {
		legacy_m_msm_camera_io_w(ub_offset << 16 | (equal_slice_ub - 1),
			vfe_dev->vfe_base + VFE40_WM_BASE(i) + 0x10);
		vfe_dev->legacy_m_ub_info->data[i] =
...
static void msm_vfe40_cfg_axi_ub(struct vfe_device *vfe_dev)
{
	...
	if (axi_data->wm_ub_cfg_policy == MSM_WM_UB_EQUAL_SLICING) {
		vfe_dev->legacy_m_ub_info->policy = MSM_WM_UB_EQUAL_SLICING;
		msm_vfe40_cfg_axi_ub_equal_slicing(vfe_dev);
	} else {
		vfe_dev->legacy_m_ub_info->policy = MSM_WM_UB_CFG_DEFAULT;
	...
	uint32_t blocking)
{
	...
	if (blocking) {
		...
		/* Halt AXI Bus Bridge */
		legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x2C0);
...
static uint32_t msm_vfe40_get_pingpong_status(
	struct vfe_device *vfe_dev)
{
	return legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x268);
}
	...
	uint32_t stats_mask, uint8_t request_comp_index, uint8_t enable)
{
	...
	comp_mask_reg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x44);
	...
	legacy_m_msm_camera_io_w(comp_mask_reg, vfe_dev->vfe_base + 0x44);
	...
	struct msm_vfe_stats_stream *stream_info)
{
	...
	/*WR_ADDR_CFG*/
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + stats_base + 0x8);
	/*WR_IRQ_FRAMEDROP_PATTERN*/
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + stats_base + 0x10);
	/*WR_IRQ_SUBSAMPLE_PATTERN*/
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + stats_base + 0x14);
}
...
static void msm_vfe40_stats_cfg_ub(struct vfe_device *vfe_dev)
{
	...
	for (i = 0; i < VFE40_NUM_STATS_TYPE; i++) {
		ub_offset -= ub_size[i];
		legacy_m_msm_camera_io_w(stats_burst_len << 30 |
	...
	uint32_t stats_mask, uint8_t enable)
{
	...
	/* CGC override */
	module_cfg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x974);
	if (enable)
		module_cfg |= cgc_mask;
	else
		module_cfg &= ~cgc_mask;
	legacy_m_msm_camera_io_w(module_cfg, vfe_dev->vfe_base + 0x974);
}
	...
	uint32_t stats_mask, uint8_t enable)
{
	...
	module_cfg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x18);
	if (enable)
		module_cfg |= module_cfg_mask;
	else
		module_cfg &= ~module_cfg_mask;
	legacy_m_msm_camera_io_w(module_cfg, vfe_dev->vfe_base + 0x18);
...
struct msm_vfe_hardware_info legacy_m_vfe40_hw_info = {
	.num_iommu_ctx = 1,
...
EXPORT_SYMBOL(legacy_m_vfe40_hw_info);

static const struct of_device_id msm_vfe40_dt_match[] = {
	{
		.compatible = "qcom,vfe40",
		.data = &legacy_m_vfe40_hw_info,
	},
	{}
};
