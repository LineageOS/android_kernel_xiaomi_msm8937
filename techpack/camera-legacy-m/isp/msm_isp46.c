...
static void msm_vfe46_init_hardware_reg(struct vfe_device *vfe_dev)
{
	...
	/* BUS_CFG */
	legacy_m_msm_camera_io_w(0x00000001, vfe_dev->vfe_base + 0x84);
	/* IRQ_MASK/CLEAR */
	...
	uint32_t irq_status0, uint32_t irq_status1)
{
	if (irq_status1 & (1 << 8)) {
		...
		legacy_m_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x374);
	}
}
...
static void msm_vfe46_process_error_status(struct vfe_device *vfe_dev)
{
	...
	if (error_status1 & (1 << 0)) {
		...
		legacy_m_msm_camera_io_dump(vfe_dev->vfe_base + 0x3A8, 0x30, 1);
	}
	...
	uint32_t *irq_status0, uint32_t *irq_status1)
{
	...
	*irq_status1 &= vfe_dev->irq1_mask;
	...
	if (*irq_status1 & (1 << 0)) {
		...
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x3D0);
	...
	uint32_t first_start, uint32_t blocking_call)
{
	...
	if (first_start) {
		legacy_m_msm_camera_io_w_mb(0x1FF, vfe_dev->vfe_base + 0x18);
	} else {
		legacy_m_msm_camera_io_w_mb(0x1EF, vfe_dev->vfe_base + 0x18);
	...
	uint8_t wm_idx, uint8_t enable)
{
	...
	val = legacy_m_msm_camera_io_r(vfe_base + VFE46_WM_BASE(wm_idx));
	if (enable)
		val |= 0x1;
	else
		val &= ~0x1;
	...
	void *arg)
{
	...
	legacy_m_msm_camera_io_w(mapped_info.paddr, vfe_dev->vfe_base + 0x268);
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	if (pix_cfg->input_mux == EXTERNAL_READ) {
		...
		legacy_m_msm_camera_io_w(temp & 0x3FFF, vfe_dev->vfe_base + 0x278);
		...
		legacy_m_msm_camera_io_w(temp, vfe_dev->vfe_base + 0x284);

		/* need to use formulae to calculate MAIN_UNPACK_PATTERN*/
		legacy_m_msm_camera_io_w(0xF6543210, vfe_dev->vfe_base + 0x288);
		legacy_m_msm_camera_io_w(0xF, vfe_dev->vfe_base + 0x2A4);
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	if (bus_sub_en) {
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x3AC);
		val &= 0xFFFFFFDF;
		val = val | bus_sub_en << 5;
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x3AC);
	...
	if (subsample_period && subsample_pattern) {
		...
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x3C8);
	} else {
		legacy_m_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x3C8);
	}

	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x39C);
	val |= camif_cfg->camif_input;
	legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x39C);
}
	...
	enum msm_isp_camif_update_state update_state)
{
	...
	if (update_state == ENABLE_CAMIF) {
		...
		legacy_m_msm_camera_io_w(val, vfe_dev->vfe_base + 0x3AC);
		legacy_m_msm_camera_io_w_mb(0x4, vfe_dev->vfe_base + 0x3A8);
		legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x3A8);
		/* configure EPOCH0 for 20 lines */
		legacy_m_msm_camera_io_w_mb(0x140000, vfe_dev->vfe_base + 0x3CC);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 1;
		/* testgen GO*/
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			legacy_m_msm_camera_io_w(1, vfe_dev->vfe_base + 0xAF4);
	} else if (update_state == DISABLE_CAMIF ||
		DISABLE_CAMIF_IMMEDIATELY == update_state) {
		msm_vfe46_config_irq(vfe_dev, 0, 0x81,
				MSM_ISP_IRQ_DISABLE);
		/* disable danger signal */
		val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0xC18);
		val &= ~(1 << 8);
		legacy_m_msm_camera_io_w_mb(val, vfe_dev->vfe_base + 0xC18);
		legacy_m_msm_camera_io_w_mb((update_state == DISABLE_CAMIF ? 0x0 : 0x6),
				vfe_dev->vfe_base + 0x3A8);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 0;
		/* testgen OFF*/
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == TESTGEN)
			legacy_m_msm_camera_io_w(1 << 1, vfe_dev->vfe_base + 0xAF4);
	}
}
	...
	uint8_t plane_idx)
{
	...
	val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + wm_base + 0xC);
	val &= ~0x2;
	...
	uint8_t plane_idx)
{
	...
	xbar_reg_cfg =
		legacy_m_msm_camera_io_r(vfe_dev->vfe_base + VFE46_XBAR_BASE(wm));
	...
	uint32_t blocking)
{
	...
	if (blocking) {
		...
		/* Halt AXI Bus Bridge */
		legacy_m_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x374);
		rc = wait_for_completion_timeout(
	...
	uint32_t blocking, uint32_t enable_camif)
{
	...
	legacy_m_msm_camera_io_w(0xFFFFFEFF, vfe_dev->vfe_base + 0x68);
	legacy_m_msm_camera_io_w(0x1, vfe_dev->vfe_base + 0x58);
	legacy_m_msm_camera_io_w_mb(0x20000, vfe_dev->vfe_base + 0x3CC);
...
static uint32_t msm_vfe46_get_pingpong_status(
	struct vfe_device *vfe_dev)
{
	return legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x2A8);
}
...
static void msm_vfe46_stats_cfg_ub(struct vfe_device *vfe_dev)
{
	...
	for (i = 0; i < VFE46_NUM_STATS_TYPE; i++) {
		ub_offset -= ub_size[i];
		legacy_m_msm_camera_io_w(VFE46_STATS_BURST_LEN << 30 |
	...
	uint32_t stats_mask, uint8_t enable)
{
	...
	stats_cfg = legacy_m_msm_camera_io_r(vfe_dev->vfe_base + 0x9B8);
	if (enable)
		stats_cfg |= stats_cfg_mask;
	else
		stats_cfg &= ~stats_cfg_mask;
	legacy_m_msm_camera_io_w(stats_cfg, vfe_dev->vfe_base + 0x9B8);
}
...
struct msm_vfe_hardware_info legacy_m_vfe46_hw_info = {
	.num_iommu_ctx = 1,
