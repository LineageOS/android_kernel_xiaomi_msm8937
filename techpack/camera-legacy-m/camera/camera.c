	...
	struct v4l2_capability *cap)
{
	...
	rc = legacy_m_msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
	if (rc < 0)
		return rc;
	...
	struct v4l2_requestbuffers *req)
{
	...
	struct msm_video_device *pvdev = video_drvdata(filep);
	unsigned int session_id = pvdev->vdev->num;
	session = legacy_m_msm_session_find(session_id);
	if (WARN_ON(!session))
		return -EIO;
	mutex_lock(&sp->lock);
	...
	struct v4l2_streamparm *parm)
{
	...
	rc = legacy_m_msm_create_stream(event_data->session_id,
...
error:
	legacy_m_msm_delete_stream(event_data->session_id,
	...
	bool valid_prio, unsigned int cmd, void *arg)
{
	...
	case MSM_CAMERA_PRIV_IOCTL_ID_RETURN_BUF: {
		...
		rc = legacy_m_msm_vb2_return_buf_by_idx(pvdev->vdev->num, sp->stream_id,
			ptr.index);
		}
		break;
	default:
...
static int camera_v4l2_vb2_q_init(struct file *filep)
{
	...
	q->mem_ops = legacy_m_msm_vb2_get_q_mem_ops();
	q->ops = legacy_m_msm_vb2_get_q_ops();
...
static int camera_v4l2_open(struct file *filep)
{
	...
	if (!atomic_read(&pvdev->opened)) {
		...
		legacy_m_msm_pm_qos_update_request(CAMERA_DISABLE_PC_LATENCY);

		/* create a new session when first opened */
		rc = legacy_m_msm_create_session(pvdev->vdev->num, pvdev->vdev);
		if (rc < 0) {
		...
		rc = legacy_m_msm_create_command_ack_q(pvdev->vdev->num,
...
post_fail:
	legacy_m_msm_delete_command_ack_q(pvdev->vdev->num, 0);
command_ack_q_fail:
	legacy_m_msm_destroy_session(pvdev->vdev->num);
session_fail:
	legacy_m_msm_pm_qos_update_request(CAMERA_ENABLE_PC_LATENCY);
...
static int camera_v4l2_close(struct file *filep)
{
	...
	struct msm_session *session;
	BUG_ON(!pvdev);
	session = legacy_m_msm_session_find(pvdev->vdev->num);
	if (WARN_ON(!session))
		return -EIO;
	...
	if (msm_is_daemon_present() != false && sp->stream_created == true) {
		...
		legacy_m_msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
	}
	...
	if (atomic_read(&pvdev->opened) == 0) {
		...
		legacy_m_msm_delete_command_ack_q(pvdev->vdev->num, 0);
		legacy_m_msm_delete_stream(pvdev->vdev->num, sp->stream_id);
		...
		legacy_m_msm_delete_command_ack_q(pvdev->vdev->num,
			sp->stream_id);
...
long legacy_m_camera_v4l2_compat_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
	long ret = 0;
...
#ifdef CONFIG_COMPAT
	.compat_ioctl32 = legacy_m_camera_v4l2_compat_ioctl,
#endif
};

int legacy_m_camera_init_v4l2(struct device *dev, unsigned int *session)
{
