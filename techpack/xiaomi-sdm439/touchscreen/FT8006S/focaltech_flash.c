...
static int fts_dpram_write_pe(u32 saddr,
		const u8 *buf, u32 len, bool wpram)
{
	int ret = 0;
	int i = 0;
	int j = 0;
	...
	FTS_INFO("write data, num:%d remainder:%d",
...
static int fts_dpram_write(u32 saddr,
		const u8 *buf, u32 len, bool wpram)
{
	int ret = 0;
	int i = 0;
...
static int fts_dram_write_ecc(const u8 *buf, u32 len)
{
	...
	FTS_INFO("dram buf length in fact:%d,offset:%d",
...
static int fts_read_file(char *file_name, u8 **file_buf)
{
	...
	snprintf(file_path, FILE_NAME_LENGTH,
		"%s%s", FTS_FW_BIN_FILEPATH, file_name);
	vts_name = getname_kernel(file_path);
	filp = file_open_name(vts_name, O_RDONLY, 0);
	if (IS_ERR(filp)) {
	...
	*file_buf = (u8 *)vmalloc(file_len);
	...
	FTS_INFO("file len:%d read len:%d pos:%d",
