...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
	...
	if ((tp_need_recovery == 1) && (lcd_need_reset == 0)) {
		...
		xiaomi_sdm439_ft8006s_fts_tp_state_recovery(ts_data);
	}

	ret = xiaomi_sdm439_ft8006s_fts_read_reg(FTS_REG_ESD_SATURATE, &val);
	if (ret < 0) {
...
static bool get_chip_id(struct fts_ts_data *ts_data)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ret = xiaomi_sdm439_ft8006s_fts_read(&reg_addr, 1, &reg_value, 1);
		if (ret < 0) {
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
	...
	if (fts_esdcheck_data.suspend == 1) {
		 ...
		 *  But in suspend, then will don't
	...
	ret = xiaomi_sdm439_ft8006s_fts_read_reg(reg_addr, &reg_value);
	if (ret < 0) {
		fts_esdcheck_data.nack_cnt++;
	...
	/* 7. get Flow work cnt: 0x91
	If no change for 5 times, then ESD and reset */
	if (!hardware_reset)
