...
static void update_firmware_release(void)
{
	if (strnstr(saved_command_line, "androidboot.mode=ffbm-01", strlen(saved_command_line))) {
		NVT_LOG("not release firmware when in ffbm mode.\n");
	} else {
		if (fw_entry) {
...
static int32_t update_firmware_request(char *filename)
{
	...
	while (1) {
		...
		if (strnstr(saved_command_line, "androidboot.mode=ffbm-01", strlen(saved_command_line))) {
			// only load firmware one time when in ffbm mode
			if (!fw_entry) {
				NVT_LOG("first time load firmware in ffbm mode.\n");
				...
				NVT_LOG("firmware is already loaded, not load again in ffbm mode.\n");
			}
		} else {
			ret = request_firmware(&fw_entry, filename, &ts->client->dev);
			if (ret) {
