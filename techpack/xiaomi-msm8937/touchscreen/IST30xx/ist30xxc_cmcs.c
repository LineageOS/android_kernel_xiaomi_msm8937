...
int ist30xx_apply_cmcs_slope(struct ist30xx_data *data, CMCS_BUF *cmcs_buf)
{
				...
				for (k = 0; k < tsp->ch_num.rx; k++) {
					...
					if (type == TSP_CH_UNKNOWN) {
						...
						cmcs_buf->spec_min[idx1] =
						    ts_cmcs->spec_item[i].
						    spec_node.buf_min[gtx_idx];
						cmcs_buf->spec_max[idx1] =
						    ts_cmcs->spec_item[i].
						    spec_node.buf_max[gtx_idx];
						gtx_idx++;
	...
	for (i = 0; i < tsp->gtx.num; i++) {
		if (tsp->gtx.ch_num[i] > height) {
			for (j = 0; j < width; j++) {
				idx1 =
				    (tsp->gtx.ch_num[i] * tsp->ch_num.rx) + j;
...
int ist30xx_cmcs_test(struct ist30xx_data *data, const u8 *buf, int size)
{
	...
	ret =
	    ist30xx_item_test(data, ts_cmcs->items, ts_cmcs->param, CMCS_CMJIT,
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	count =
	    sprintf(msg,
		    ...
		    tkey->ch_num[2].rx, 255, 255, 255, 255);
	strcat(buf, msg);
...
static ssize_t opentest_proc_write(struct file *filp,
				   const char __user *userbuf, size_t count,
				   loff_t *ppos);
static ssize_t opentest_proc_read(struct file *file, char __user *buf,
				      ...
				      size_t count, loff_t *ppos)
{
	...
	*ppos += count;
	...
	return sprintf(ptr, "%02X%02X%02X%02X%02X%02X%02X%02X\n",
		       Lockdown_Info_High.lockinfo[3],
		       Lockdown_Info_High.lockinfo[2],
		       Lockdown_Info_High.lockinfo[1],
		       Lockdown_Info_High.lockinfo[0],
		       Lockdown_Info_LOW.lockinfo[3],
		       Lockdown_Info_LOW.lockinfo[2],
		       Lockdown_Info_LOW.lockinfo[1],
		       Lockdown_Info_LOW.lockinfo[0]);
...
void create_tp_proc(void)
{
	...
	opentest =
	    proc_create(IST_PROC_ITO_OPENTEST_DEBUG, ITO_TEST_AUTHORITY,
			msg_tp_test, &proc_opentest_file_fops);
	if (NULL == opentest) {
